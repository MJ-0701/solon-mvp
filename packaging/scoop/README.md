...
manifest 는 `bin\\sfs.cmd` 를 통해 `sfs` 를 노출합니다. wrapper 는 먼저 Windows native
read-only 명령(`version`, `status`, `context cat/path`, `guide`)을 `sfs.ps1` 로 처리하고,
나머지 명령도 `sfs.cmd -> sfs.ps1 -> bin/sfs` bridge 로 넘깁니다. raw Git Bash `%*` 직행
경로는 Windows 기본 경로가 아닙니다.
...
bucket manifest 를 렌더링한 뒤 아래 흐름을 실행합니다. 0.6.40 기준 smoke 는 먼저 로컬 이전
