...
current_wu: WU-38
current_wu_path: 2026-04-19-sfs-v0.4/sprints/WU-38.md
...
current_wu_owner: codex-release-channel-verify-20260502
...
resume_hint:
  default_action: |
    ...
    3) Continue WU-38 release-channel verification hotfix if 0.5.61-product is
       not yet deployed. Otherwise ask user for the next WU/domain.
  on_skip_patterns: ["아니", "잠깐", "다른", "stop"]
  on_skip_action: "What do you want to do instead (1 line)?"
  on_ambiguous: "WU-38 release-channel hotfix may still be pending. Continue release?"
  safety_locks:
...
- Found the 0.5.60 release completion gap: remote Homebrew tap was updated, but the actual local Homebrew tap clone was stale at `0.5.57-product`. Local tap was pulled and global `sfs` is now `0.5.60-product`.

## ② In-Progress

- WU-38 hotfix is being prepared for `0.5.61-product`: Homebrew self-upgrade tap refresh + release verifier.

## ③ Next

- Commit WU-38, release `0.5.61-product`, update Homebrew + Scoop, run `verify-product-release.sh --version 0.5.61-product`, then release mutex.
