...
|:--:|:---|:--:|:---|:---|:---|
| **WU-25** | #1 sfs slash command 구현 part 2 — `/sfs plan` + `/sfs review` (gate verdict 통합, gates.md §1 7-enum enforce). spec only (계획만), 실 구현은 다음 세션. | pending | 2026-04-28T00:30+09:00 | brave-gracious-mayer (24th) | [WU-25.md](WU-25.md) |
| **WU-26** | #1 sfs slash command 구현 part 3 — `/sfs decision` (mini-ADR 5 섹션) + `/sfs retro --close` (sprint close + auto commit, push manual §1.5 정합). spec only (계획만), 큰 WU 병렬 진행 위해 신설 (D-H-WU-26 = D-G-WU-25 와 동시 active 가능). | pending | 2026-04-28T00:50+09:00 | brave-gracious-mayer (24th) | [WU-26.md](WU-26.md) |
