...
`docset version: v0.4-r4` · `last_updated: 2026-04-28` · `status: draft (design)` · `audience: 채명정 본인 (Phase 1) → 자영업자/솔로 창업가 (Phase 2)`
...
이 폴더(`2026-04-19-sfs-v0.4/`) 는 **Solon 제품의 설계 Docset v0.4-r4** 다. 코드가 아니라 **설계 문서의 집합** 이며, Phase 1 구현은 이 docset 을 스펙으로 삼아 별도 저장소에서 진행된다.
...
│  │                                                          │    │
│  │  PDCA = P-1 + Plan + Design + Do + Check + Act            │    │
│  │                                                          │    │
│  │  Gate: G-1 / G0 / G1 / G2 / G3 / G4 / RELEASE / G5       │    │
│  │                                                          │    │
...
### Foundation Invariants (v0.4-r4)

2026-04-28 구조 리뷰에서 원칙 번호를 늘리기보다, 실행 산출물 계약을 단단하게 만드는 비번호 불변식을 추가했다.

| ID | 불변식 | 의미 |
|----|--------|------|
| FI-1 | Artifact Contract First | agent 수보다 산출물 입력/출력/검증 계약을 먼저 고정 |
| FI-2 | Taxonomy Is Root | 디자인/프론트/문서는 택소노미를 기준으로 정렬 |
| FI-3 | Shift-left Infra/Security | 인프라/보안은 배포 직전 점검이 아니라 설계 초반부터 입력 |
| FI-4 | Continuous Documentation Ledger | 문서는 마지막 산출물이 아니라 매 단계의 decision/evidence ledger |
...
Initiative                  (수개월, 전략 단위, 1 Initiative = 1 G0 Brainstorm)
  └─ Sprint                 (1~2주, 실행 단위, N개 PDCA 묶음)
       └─ PDCA              (1~수일, 기능 단위, 1 PDCA = Plan→Design→Do→Check→Act)
```
...
### 5.3 Gate 집합 (v0.4-r4 = G-1/G0/G1~G5 + RELEASE)
...
| RELEASE Readiness | production 배포 전 | infra + security + QA + 사람 | Sonnet/Opus 혼합 | secret/auth/data/monitoring/rollback/cost 체크 |
...
> Release Readiness 는 v0.4-r4 기준 gate vocabulary 로 먼저 고정했고, 별도 command 로 둘지 `/sfs check` 옵션으로 흡수할지는 미결이다.
...
내부 gate dispatcher 는 사용자-facing slash command 로 세지 않는다. 각 command 의 embedded evaluator 호출 계약으로만 존재한다.
...
| Area | 상태 | 비고 |
|------|------|------|
| Docset v0.4-r4 | 🟢 설계 보강 완료 (이 문서 포함) | Round 4 = startup team-agent artifact/gate foundation 보강 |
...
| L3 driver (notion plugin) | 🔴 Phase 2 | Phase 1 기본은 `none`; Notion 은 외부 공유 필요 시 |
...
├── 05-gate-framework.md               G-1 + G0 + G1~G5 + Release Readiness + 7 Failure Modes
...
│
└── appendix/
    ├── commands/                      ← /sfs * 13개 command spec (+ README)
...
| **Gate** | phase 전환점 검증 관문 (G-1/G0/G1/G2/G3/G4/RELEASE/G5) |
| **Release Readiness** | G4 개발/QA 수락 이후, production 배포 전 secret/auth/data/monitoring/rollback/cost 를 확인하는 별도 관문 |
...
- 현재 = `v0.4-r4` (v0.4 → Round 4 foundation hardening: Artifact Contract, Taxonomy root, Release Readiness, AGENTS bootstrap 정합)
...
| **v0.4-r4** | **2026-04-28** | **기초공사 구조 보강 — AGENTS bootstrap shim 신설, startup team-agent 13-step artifact chain, Foundation Invariants(FI-1~4), G0/Release Readiness gate vocabulary 정합, schema enum 정합, MVP 7-step projection 경계 명시, 구조 리뷰 문서 2건 추가** |

### 12.4 다음 Round 예상

- v0.4-r5 (예상): Phase 1 dogfooding 피드백 반영 (실제 구현 중 발견된 원칙 충돌 / 누락 spec)
...
| "왜 이렇게 설계했는지 알고 싶다" | [§00 제품 철학](00-intro.md) → [§02 13원칙 + Foundation Invariants](02-design-principles.md) |
