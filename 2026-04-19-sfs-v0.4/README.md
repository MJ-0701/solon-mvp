...
4. **§10-phase1-implementation.md** — 실제 10일 구현 플랜
...
| `/sfs install` (Socratic wizard 포함) | 🔴 미구현 | 10일 플랜 D1 |
| `/sfs brainstorm`/`plan`/`design` | 🔴 미구현 | 10일 플랜 D2-D5 |
| `/sfs do`/`handoff`/`check` | 🔴 미구현 | 10일 플랜 D6-D8 |
| `/sfs retro`/`escalate`/`division` | 🔴 미구현 | 10일 플랜 D9 |
| L1/L2 channel | 🟡 스펙 완료 | bkit 재활용 가능 |
| L3 driver (notion plugin) | 🔴 Phase 2 | Phase 1 에선 `none` |

10일 세부 플랜은 [§10-phase1-implementation.md](10-phase1-implementation.md) 참조.
...
├── HANDOFF-next-session.md            ← 다음 세션 작업 지시 (로컬 전용, git 제외)
│
...
├── 10-phase1-implementation.md        10일 구현 플랜 + 8 Evaluator 상세
│
└── appendix/
