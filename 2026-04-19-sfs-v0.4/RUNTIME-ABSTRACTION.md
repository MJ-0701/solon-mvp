...
defines:
  ...
  - concept/reasoning-tier-contract                        # 🆕 2026-05-01
  - rule/c-level-strategy-worker-execution-split           # 🆕 2026-05-01
references:
...
triggered_by_user_directive: |
  ...
  "설계같은 작업이 무겁고 또 엄청 중요한 작업을 할때는 AI agent 모델을 높은걸 쓰는게 맞는데 ...
  단순 코드구현일때는 subagent로 opus가 아니라 sonnet 정도의 모델을 사용해도 충분하거든??
  ... C레벨은 방향성과 설계를 주도하는 애들이지 실제로 실무를 뛰는 애들이 아니야 ...
  이 기능도 추상화 시켜서 LLM Agent 별 맞춤 구현할 수 있도록 셋팅해줘"
  — 채명정, 2026-05-01 (runtime별 subagent/model tier 추상화 지시)
---
...
- §5.4 Reasoning Tier Contract — C-Level/worker 모델 할당 추상화
...
| Opus / Sonnet / Haiku tier 할당 | `03-c-level-matrix.md §3.6`, `07 §7.2`, plugin.json | Anthropic 모델명. 2026-05-01 부터 §5.4 의 `strategic_high` / `review_high` / `execution_standard` / `helper_economy` 로 추상화하고, runtime adapter 가 concrete model/profile 을 resolve |
...
4. `agents/*.md` frontmatter 의 `model:` 필드 — §5.4 reasoning tier 로 단계적 migration 필요.
...
- **모델 선택**: "Opus vs Sonnet vs Haiku" 같은 runtime-specific 모델명은 L2 의 책임. L1 은 `strategic_high` / `execution_standard` / `helper_economy` 같은 추상 reasoning tier 만 노출.
...
### §5.4 Reasoning Tier Contract — C-Level 은 설계, Worker 는 실행

`concept/reasoning-tier-contract`, `rule/c-level-strategy-worker-execution-split`

사용자 지시의 핵심은 **역할과 모델 비용을 같은 축에 두지 않는 것**이다. C-Level 은 방향성 / 설계 / 충돌 조정 / 최종 판단을 담당하고, 실무 worker 는 이미 고정된 plan / architecture / AC 를 구현한다. 따라서 L1 계약은 concrete 모델명이 아니라 다음 reasoning tier 를 기록한다. 단, 사용자가 프로젝트 모델 설정을 하지 않거나 거부하거나 잊은 경우에는 workflow 를 막지 않고 **현재 runtime 에서 사용자가 선택한 모델/effort** 를 그대로 쓴다 (`current_model` fallback).

| reasoning_tier | 기본 역할 | 사용 시점 | runtime별 예시 (비규범) |
|---|---|---|---|
| `strategic_high` | CEO / CTO / CPO, 큰 architecture 결정, 제품 방향성, public contract 변경 | 설계가 아직 열려 있거나 실패 비용이 큰 판단 | Claude Opus 계열, Codex GPT-5.5 계열, Gemini highest-reasoning profile |
| `review_high` | CPO / Evaluator, gate verdict, risk review | 자기검증 방지용 fork/read-only 검증. 단순 schema-only review 는 `execution_standard` 로 낮출 수 있음 | 각 runtime 의 high-reasoning evaluator profile |
| `execution_standard` | Implementation worker / 본부 실무자 / 정해진 slice 구현 | plan, architecture, AC, file scope 가 고정된 뒤의 코드 작성과 테스트 보강 | Claude Sonnet 계열, Codex standard/coding profile, Gemini balanced profile |
| `helper_economy` | parser / formatter / grep summary / JSON 변환 | 의미 판단이 거의 없는 deterministic 보조 작업 | Claude Haiku 계열, Codex mini profile, Gemini flash/economy profile |

#### Project model profile / current fallback

Product 배포판의 프로젝트별 설정 SSoT 는 `.sfs-local/model-profiles.yaml` 이다. 이 파일은 `selected_runtime`, `selected_policy`, `agent_defaults`, `agent_model_overrides` 를 보관한다.

- 기본 fallback 은 `selected_policy: current_model` — agent 별 모델을 별도 지정하지 않으면 현재 Claude/Codex/Gemini/custom host 에서 사용자가 켜둔 모델을 그대로 쓴다.
- Solon 의 `strategic_high` / `review_high` / `execution_standard` / `helper_economy` 는 비용/품질 권장값이지 hard block 이 아니다.
- 사용자가 worker/helper 까지 high-end 모델을 쓰겠다고 명시하면 `selected_policy: all_high` 또는 agent별 override 로 허용한다.
- GPT/Codex 계열처럼 `model + reasoning_effort` 조합이 필요한 runtime 은 둘 다 기록한다 (예: `gpt-5.5` + `xhigh`/very_high). Claude 계열처럼 `opus-4.7`, `opus-4.6`, `sonnet`, `haiku` 식의 모델명이 중심인 runtime 은 concrete model 명을 기록한다.

#### Worker 가 `execution_standard` 를 써도 되는 경계

다음 조건을 모두 만족할 때만 낮은 비용의 implementation worker 를 기본값으로 쓴다.

1. CEO/CTO/CPO 또는 plan 문서가 작업 방향, architecture, acceptance criteria, files_scope 를 이미 고정했다.
2. worker 의 임무가 "설계 변경" 이 아니라 "정해진 설계의 구현 / 테스트 / 작은 리팩터" 로 제한된다.
3. CPO/Evaluator 는 별도 instance/tool 로 유지되어 worker 가 자기 산출물을 pass 처리하지 않는다.

#### 즉시 high tier 로 escalate 하는 트리거

- architecture / public API / data model / security / data-loss risk 변경.
- plan 또는 AC 가 모호해서 worker 가 의미 결정을 해야 하는 상황.
- 같은 test 또는 review finding 이 2회 이상 반복 실패.
- files_scope 밖 변경이 필요해진 상황.
- 사용자 의미 결정(A/B/C)이 필요한 갈림길.

#### Audit fields

runtime adapter 는 subagent 를 spawn 하거나 review 를 실행할 때 최소한 다음 필드를 log/review/queue artifact 에 남긴다.

```yaml
reasoning_tier: execution_standard
runtime: current|claude|codex|gemini|custom
resolved_model: "<runtime concrete model/profile>"
reasoning_effort: "<runtime effort/profile or current>"
override_reason: null
approver: "lead|cto|cpo|user"
```

`resolved_model` 은 감사용 기록일 뿐 SSoT 가 아니다. 실제 매핑은 각 runtime adapter 의 profile registry 가 담당하고, 이 문서는 tier 의미만 고정한다.
...
- **Model tier 는 runtime별로 resolve**. queue/review/implement artifact 는 `reasoning_tier` 와 `resolved_model` 을 함께 기록한다. Solon core 는 tier 의미만 보고, Claude/Codex/Gemini adapter 가 concrete 모델/profile 로 변환한다.
...
3. **2026-05-01 v0.3-agent-model-tier**: C-Level/worker 모델 할당을 concrete 모델명에서 runtime-agnostic reasoning tier 로 승격. C-Level = 설계/방향, worker = 고정 plan 구현. runtime adapter 가 concrete model/profile resolve.
4. **후속**: SDK/plugin 수준 `appendix/runtime-adapters/{codex,gemini-cli}/` 는 실제 병목 확인 후 작성.
