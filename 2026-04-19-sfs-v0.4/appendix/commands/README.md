...
> Solon 플러그인이 Claude Code에 노출하는 **Slash Command 12종**의 단일 레지스트리.
...
| 7 | `/sfs check` | Check | 공통 | `quality/qa/lead` + `strategy/cpo` | Do 완료 후 G4 호출 |
...
총 12개 — 기존 11개(#0, #2~#11)에 brownfield 지원을 위해 `#1 /sfs discover`를 v0.4-r2에서 추가.
...
- `appendix/commands/plan.md` ~ `retro.md` — 나머지 10개 spec
