...
21. **운영 환경 매핑 (Multi-runtime role split)** — Claude Code / Cowork / Codex / Gemini-CLI 의 환경별 역할 분리는 [`RUNTIME-ABSTRACTION.md`](./RUNTIME-ABSTRACTION.md) §6.5 본체 참조. 본 §1 은 진입 인지만 보장 (도구 강제 X), 강제력은 §1.13 R-D1 + `scripts/cut-release.sh` divergence 감지 자동화가 담당. 도구 추가/변동 시 §1 mutate 0 — RUNTIME-ABSTRACTION §6.x adapter 슬롯 + §6.5 매핑 row 만 갱신. 근거: 26th 세션 `eager-festive-shannon` (2026-04-30) 사용자 (i)+(iii-b) 하이브리드 결정 + AGENTS bootstrap shim (v0.4-r4 변경점) + 2026-04-20 사용자 직접 지시 verbatim ("sfs를 claude 뿐만 아니라 codex랑 gemini-cli에서도 사용하고 싶거든??", WU-8). 본 §1.21 신설 자체가 §1.4 Option β minimal cleanup default 적용 case (§1 line +1줄 + 본체는 부록성 문서로 분리 = §1.14 분리 우선순위 정합).
