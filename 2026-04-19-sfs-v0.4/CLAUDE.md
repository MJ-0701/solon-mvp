...
5. **Git lifecycle agent-owned (branch/commit/merge/push)** — 보통 사용자는 브랜치를 직접 만들지 않고 작업만 발화한다. AI 는 §1.22 Git Flow 에 따라 도메인 브랜치 생성/전환, file edit, 검증, `git add`, commit, branch push, main merge/PR, main push 까지 작업 완료 조건으로 수행한다. 예외: destructive git (`reset --hard`, checkout discard), unrelated dirty set, merge conflict, failing tests, protected branch/remote rejection, auth prompt 는 즉시 stop + 사용자 결정 대기.
...
11. **Session resume protocol (WU-28 simplify)** — 세션 시작 직후 `PROGRESS.md` frontmatter `resume_hint` 필드 **필수 확인**. **첫 사용자 발화 = `default_action` 자동 실행** (default behavior, trigger word 매칭 단계 폐지). `on_skip_patterns` (예: '아니 / 다른 작업 / 잠깐' 등 명시적 분기 발화) 만 skip + `on_skip_action` 가이드 표시. 모호 시 `on_ambiguous` (1-line 확인 Q, 자율 해석 금지). `safety_locks` 는 발화 종류와 무관하게 **항상 우선** · 원칙 2 위반 action 은 `default_action` 안에 들어 있어도 자동 실행 절대 금지 (정지 + 사용자 대기). 운영 helper: `scripts/auto-resume.sh` (default_action 추출 + 출력, WU-28 신설).
