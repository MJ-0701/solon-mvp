...
22. **Git Flow branch-first 운영** — 새 작업 시작 전 `git status --short` + branch 확인 → 도메인별 `feature/*` 또는 `hotfix/*` 생성/전환. unrelated dirty 혼입 시 stop. 완료 조건 = 검증 통과 + commit + push + main 흡수 + origin main 반영.
