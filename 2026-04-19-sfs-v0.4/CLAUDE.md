...
26. **Bug fix completion contract** — 개발단계 버그 수정은 임시 복구만으로 완료 금지. 완료 조건 = 즉시 복구 + 원인 제거 + 재발 방지 검증/게이트 + 문서/핸드오프 반영. 단기 mitigation 은 명시적으로 debt 기록.
27. **SFS 6 철학 SSoT** — 6 철학 (Grill Me / Ubiquitous Language / TDD-no-overtake / Deep Module / Gray Box / Daily System Design) 의 본문 SSoT 는 [`SFS-PHILOSOPHY.md`](./SFS-PHILOSOPHY.md). 본 CLAUDE.md 에 본문 복제 금지 (SSoT 이중화 방지).
28. **Cowork sandbox git 작업 금지** — Cowork 환경에서는 sandbox 안 git 명령 (`git clone`/`commit`/`bundle`/`push` 등) **시도 자체 금지**. push 권한 부재 + FUSE stale lock 등으로 어차피 실패 → 토큰 낭비 + 사용자 wait. 변경 산출물은 file write 만 하고 commit/push 는 사용자 host terminal 수동 실행 (§1.18 command block 안내). Codex / Claude Code / 기타 dev runtime 은 §1.5 Git lifecycle agent-owned 그대로 유효 — 본 §1.28 은 **Cowork 한정 예외**. 근거: 2026-05-03 affectionate-trusting-thompson 세션 사용자 명시 ("어차피 계속 실패되는거 그냥 나한테 수동으로 하라고 안내할것").
29. **Cowork ↔ Code mode work split + trigger phrases** — Claude 발화 수용 시 다음 작업을 항상 두 분류로 split + 안내: **(C-Cowork)** sandbox file write 중심 — skeleton/plan/doc/PROGRESS·implement·log·CLAUDE.md 갱신, brainstorm/plan/review-g* 작성, AC 정합 grep 검증, 단순 bash dry-run. **(D-Dev=Claude Code/Codex/Gemini)** full git lifecycle (§1.5) + 대량 bash 실 logic 구현 + 실 test 실행 + CI invoke + brew/scoop sha256 갱신 + cross-instance evaluator invoke + protected branch push. 동일 mode 연속 작업은 **batch 우선** (work→code→work→work→code→work 금지 → work→work→code→code→work→work 권장 또는 가능 시 한 mode 안에서 한 번에 끝내기). 결정 갈림길 (mode 불명) 시 §1.3 self-validation-forbidden — 사용자에게 mode 선택 위임. **모든 next-action 안내 시 다음 mode 진입 트리거 발화 명령어 (user prompt 예시 ≥1, 가능 시 2~3개 alias) 를 함께 제시** — Code → Cowork / Cowork → Code 전환 마찰 최소화 + PROGRESS.md `resume_hint.trigger_phrases` 필드에 동일 phrase 등록 (next session auto-resume 정합). 근거: 2026-05-04 amazing-keen-euler 세션 사용자 명시 ("앞으로 claude에서 작업할때 이걸 나눠줘" + "발화명령어도 자동으로 추가해줘 next action에 code ↔ cowork간 왔다갔다 하면서 작업해야되니까").
