...
23. **SFS 문서 생명주기 하네스** — `brainstorm/plan/implement/log/review` 는 진행 중 workbench(노트패드/화이트보드) 로만 두고, 완료 시 `/sfs report` 로 결론·결정·검증·리스크 중심 `report.md` 를 만든 뒤 사용자 동의가 있는 `--compact`/`retro --close` 에서 workbench 문서를 redirect stub 로 압축한다. 히스토리 서사는 `retro.md`, session log, events 가 담당한다. 근거: 2026-05-01 사용자 결정 ("완료 후 실제 작업문서는 보고서로 갱신", "이전 문서는 사용자 동의하에 최종보고서로 정리").
