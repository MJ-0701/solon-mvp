...
18. **Commit 안내 형식 의무 (Copy-paste ready)** — AI 가 사용자 manual commit 안내 시 **"권장 message: <text>" 같은 prose 형식 금지**. 반드시 사용자가 터미널에 그대로 복붙 가능한 **shell command block** 으로 제공: ① `cd ~/agent_architect` → ② `git add <files…>` (multi-line `\` continuation 또는 인자 나열) → ③ `git commit -m "<wip-tag>" -m "<상세 narrative>"` (멀티라인 = `-m` 다중 또는 heredoc) → ④ optional `# git push origin main` (사용자 결정 시점, 주석 처리). §1.5' (commit 자체 사용자 manual) 은 보존 — Claude 는 commit 실행 안 함, 그러나 안내는 즉시 실행 가능한 shell form. 근거: 25th-1 admiring-zealous-newton 사용자 결정 ("이것도 명시해줘") — prose 형식 → 사용자가 message 직접 조립 → 시간 낭비 + 오타 위험. **본 §1.18 신설 자체가 첫 적용 case** (sub-task 1 commit 명령 = §1.18 형식 그대로 출력).
