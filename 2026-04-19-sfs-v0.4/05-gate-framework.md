...
title: "§5. Gate Framework G-1 + G0 + G1~G5 + Release Readiness + 7 Failure Modes"
version: 0.4-r4
status: draft
...
# §5. Gate Framework G-1 + G0 + G1~G5 + Release Readiness + 7 Failure Modes
...
>
> **2026-04-28 구조 리뷰 반영**: G0 Brainstorm Gate 와 Release Readiness Gate 를 본문 gate vocabulary 에 명시한다.
> G0는 Initiative 진입 gate 이고, Release Readiness 는 Production 전 운영·보안·롤백 gate 이다. 둘 다 G1~G5 PDCA 내부 gate 를 대체하지 않는다.

---

## TOC

- 5.1 Gate 매트릭스 (G-1 + G0 + G1~G5 + Release Readiness)
...
| **G0** | Brainstorm Gate — Initiative 문제·대안·성공 지표 구체성 | `strategy/ceo` | 새 Initiative 선언 직후, Sprint decomposition 전 | 공통 | Plan(G1) 진입 허가 | CPO review + Sonnet × 1회 |
...
| **G4** | Check Gate — 실제 vs 설계 gap (정량+정성) | `qa/lead` + `strategy/cpo` | Do 완료 후 | 공통 | Act 진입 OR Escalate | Sonnet × 3~5회 + Opus × 1회 |
| **Release Readiness** | Production 전 운영·보안·롤백 준비성 | `infra/lead` + `strategy/ceo` | Dev deploy + G4 후, production deploy 직전 | 공통 | Production deploy 승인 | Sonnet × 1~3회 + human review |
...
### 5.1.6 Release Readiness 를 G4 와 분리하는 이유

G4 는 "구현 결과가 설계와 맞는가"를 보는 Check Gate 다. Production 전에는 여기에 더해 운영·보안·롤백·비용·모니터링 준비성이 필요하다. 이 항목을 G4 안에 숨기면 QA 통과와 배포 승인 사이의 책임 경계가 흐려진다.

따라서 Release Readiness 는 다음 규칙을 따른다.

- G4 PASS 또는 명시적 deferred decision 이후에만 실행한다.
...
| **G0** | `intent-discovery-validator` + CPO review | taxonomy seed check | design opportunity check | tech feasibility sketch | risk framing check | infra/security risk sketch |
...
gate_call:
  ...
  sprint_id: "SP-005"
  pdca_id: "PDCA-042"
  gate_id: "G3"                      # G-1 | G0 | G1 | G2 | G3 | G4 | G5 | RELEASE
  target_division: "dev"
...
| **qa** | `G4 = test-coverage-delta × 0.3 + defect-leak × 0.1 + 5-Axis × 0.6` | 타 본부 상호검증 필요 없음 — qa의 산출물은 **타 본부의 Do 결과 자체를 검증한 리포트**이므로 메타-self-validation 문제 없음 | defect-leak = 테스트 후 프로덕션 버그 수 (역지표) |
...
| **G0** | ✅ | ✅ | 🚨 reject/re-scope Initiative | ✅ | ✅ | ✅ | 🚨 Initiative 보류 |
...
- **G0의 FAIL-HARD = Initiative reject/re-scope**: 구현이 아니라 문제 정의가 부족한 상태이므로, PDCA로 밀어 넣지 않는다.
...
Release Readiness Gate 는 G4 이후, production deploy 직전에 실행한다.

```
G4 PASS or deferred decision
...
이 gate 는 G4를 대체하지 않는다. G4는 "결과가 설계와 맞는가"이고, Release Readiness 는 "운영 환경에 올려도 되는가"다.
