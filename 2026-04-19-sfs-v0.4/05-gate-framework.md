...
title: "§5. Gate Framework Gate 1~7 + Release Readiness + 7 Failure Modes"
version: 0.4-r4
status: draft
...
defines:
  ...
  - concept/release-readiness-gate
...
# §5. Gate Framework Gate 1~7 + Release Readiness + 7 Failure Modes
...
>
> **v0.4-r2 변경 (brownfield 대응)**: Gate 집합은 사용자 표기 기준 **Gate 1~7**로 정리한다.
> **Gate 1 Intake**는 §4.3에서 정의된 P-1 Discovery Phase의 종료 게이트로, `discovery-report.md`의 완성도와 **사람 최종 승인**(원칙 10 `human-final-filter`, 원칙 11 `brownfield-first-pass`)을 확인한다.
> Gate 1은 PDCA 사이클 내부 Gate가 아니라 **install 시점의 1회성 intake Gate**이므로 별도 섹션(§5.11)에서 상세 명세한다.
>
> **2026-04-28 구조 리뷰 반영**: Gate 2 Brainstorm 과 Release Readiness Gate 를 본문 gate vocabulary 에 명시한다.
> Gate 2는 Initiative 진입 gate 이고, Release Readiness 는 Production 전 운영·보안·롤백 gate 이다. 둘 다 Gate 3~7 PDCA 내부 gate 를 대체하지 않는다.
>
> **2026-05-02 사용자 표기 정리**: Solon report / 사용자 안내 / 새 CLI 예시는 `Gate 1..7` 숫자를 사용한다.
> 기존 storage id 는 오래된 frontmatter·events 호환용으로만 유지한다.

---

## TOC

- 5.1 Gate 매트릭스 (Gate 1~7 + Release Readiness)
- 5.2 Gate별 Evaluator 매핑 (본부별)
- 5.3 Gate Call Contract (입력 표준)
- 5.4 GateReport Schema (출력 표준)
- 5.5 Gate 6 Formula (본부별 가중치)
- 5.6 5-Axis CPO 평가 (Gate 6 공통)
- 5.7 7 Failure Modes 정의
- 5.8 Gate별 Failure Mode 허용 매트릭스
- 5.9 Gate 실행 타임라인 (PDCA 위에 겹쳐보기)
- 5.10 Gate와 §6 Escalate의 연결점
- 5.11 Gate 1 Intake 상세 명세 (brownfield 전용)
- 5.12 Release Readiness Gate (production 전용, lightweight)
...
| 사용자 표기 | 번호 | legacy storage id | 의미 | 오퍼레이터 | 트리거 시점 | 적용 모드 | 통과 시 다음 단계 | 비용 예산 (Phase 1) |
|:---:|:---:|:---:|------|-----------|----------|:---:|----------------|-----------------|
| **Gate 1 (Intake)** | **1** | G-1 | Intake Gate — discovery-report 완성도 + **사람 최종 승인** (§5.11) | `strategy/ceo` | `install.sh --mode brownfield` 직후 P-1 완료 시점 | brownfield only | P-1 종료 → Plan 진입 허가 | Sonnet × 2~3회 (validator) + **human review time** |
| **Gate 2 (Brainstorm)** | **2** | G0 | Brainstorm Gate — Initiative 문제·대안·성공 지표 구체성 | `strategy/ceo` | 새 Initiative 선언 직후, Sprint decomposition 전 | 공통 | Plan 진입 허가 | CPO review + Sonnet × 1회 |
| **Gate 3 (Plan)** | **3** | G1 | Plan Gate — 요구사항·AC 측정 가능성 | `strategy-pm/lead` | Plan 문서 작성 완료 시 | 공통 | Design 진입 허가 | Opus × 1회 + Sonnet × N회 |
| **Gate 4 (Design)** | **4** | G2 | Design Gate — 설계 완성도 | 각 본부장 (lead) | Design 문서 작성 완료 시 | 공통 | Do 진입 허가 | Sonnet × 2~3회 |
| **Gate 5 (Handoff)** | **5** | G3 | Pre-Handoff Gate — 산출물 핸드오프 가능성 (binary) | 각 본부장 (lead) | Do 완료 직전 | 공통 | 핸드오프 OR Check 진입 | Sonnet × 1~2회 |
| **Gate 6 (Review)** | **6** | G4 | Check Gate — 실제 vs 설계 gap (정량+정성) | `qa/lead` + `strategy/cpo` | Do 완료 후 | 공통 | Act 진입 OR Escalate | Sonnet × 3~5회 + Opus × 1회 |
| **Release Readiness** | n/a | RELEASE | Production 전 운영·보안·롤백 준비성 | `infra/lead` + `strategy/ceo` | Dev deploy + Gate 6 후, production deploy 직전 | 공통 | Production deploy 승인 | Sonnet × 1~3회 + human review |
| **Gate 7 (Retro)** | **7** | G5 | Sprint Retro — 학습 루프 (정성) | `strategy/ceo` + `strategy-pm/lead` | Sprint 종료 시 | 공통 | 다음 Sprint 반영 | Opus × 1~2회 |
...
2. **표준성** — 입력(`schema/gate-call-contract`)·출력(`schema/gate-report-v1`)이 gate 종류에 관계없이 **동일 스키마**. 따라서 L2 SSoT에 동형 저장, L3 driver view 단일 쿼리로 집계 가능.
3. **분리성** — Gate 통과 여부 판단은 **Evaluator**, Gate 실행 의사결정(언제 호출/누구에게/재시도)은 **오퍼레이터**. 이 둘은 다른 모델·다른 agent.

### 5.1.2 왜 Gate 5 (Handoff)을 따로 두었는가
...
- Gate 5 FAIL → 해당 본부 내 재작업, Gate 6까지 가지 않음 (비용 절약)
- Gate 5 PASS → Do 완료 처리 → Gate 6 Review 진입

Gate 5는 본부 **내부** 완료 조건, Gate 6는 **전사** 품질 조건이라는 것이 핵심 구분이다.

### 5.1.3 왜 Gate 7 (Retro)가 필요한가

Gate 3~6은 PDCA **단일 사이클** 내부 게이트. Gate 7은 **Sprint = N개 PDCA** 전체에 대한 **학습 게이트**:
- 6 본부 × N PDCA의 결과를 모아 패턴을 추출
- Gate FAIL 빈도·대기시간·비용 집계
- 실패 패턴 → `memory/learnings-v1.md` 축적 (H6 학습 루프, §6 참조)

Gate 7 결과는 **다음 Sprint의 Plan 입력**으로 연결된다 (Sprint 간 학습 전파).

### 5.1.4 왜 Gate 1 (Intake)이 필요한가 — brownfield 전용 게이트

`concept/gate-1-intake`: **`install.sh --mode brownfield` 직후 P-1 Discovery Phase의 종료점에서 실행되는 intake 게이트**. Gate 3~7이 PDCA 사이클 **내부** 게이트라면, Gate 1은 "Solon을 아직 투입할지 말지" 자체를 결정하는 **경계 게이트**이다.

네 가지 이유로 Gate 1을 Gate 3과 합치지 않고 분리한다:

1. **적용 모드 비대칭** — greenfield는 P-1이 없으므로 Gate 1을 거치지 않는다(`rule/greenfield-vs-brownfield-entry`, §4). Gate 3과 합치면 greenfield flow에 불필요한 분기 생성.
2. **오퍼레이터가 다름** — Gate 3은 `strategy-pm/lead` 주도(요구사항 측정성 판단), Gate 1은 `strategy/ceo` 주도(전략적 "투입할 가치가 있는가" 판단 + 최종 승인).
3. **사람 승인이 필수** — Gate 1은 원칙 10(`human-final-filter`)을 가장 강하게 요구한다. signature 파일에 **사용자 이름·날짜·6개 체크박스 서명**이 없으면 PASS 불가. Gate 3은 자동 pass 가능.
4. **1회성 (per install)** — Gate 1은 Solon 도입 시 **1회**만 실행된다(`rule/p-1-run-once-per-install`, §4.3.10). Gate 3~7은 매 Sprint × 매 PDCA마다 반복. Gate 빈도·집계 로직이 전혀 다름.

### 5.1.5 Gate 집합의 모드별 가시성

| 모드 | 실행되는 Gate | 비고 |
|------|---------------|------|
| **greenfield** | Gate 2 → Gate 3 → Gate 4 → Gate 5 → Gate 6 → Gate 7 | Gate 1 skip |
| **brownfield** | **Gate 1 (Intake)** → Gate 2 or direct-to-Gate 3 → Gate 3 → Gate 4 → Gate 5 → Gate 6 → Gate 7 | Gate 1 PASS 후 Gate 2 호출 여부는 원칙 12(`brownfield-no-retro-brainstorm`)로 제한됨 |

→ 두 모드에서 공통적으로 나타나는 게이트(Gate 3~7)는 본 섹션 이하에서 통합 기술하고, Gate 1 고유 스펙은 §5.11로 격리.

### 5.1.6 Release Readiness 를 Gate 6 과 분리하는 이유

Gate 6 는 "구현 결과가 설계와 맞는가"를 보는 Check Gate 다. Production 전에는 여기에 더해 운영·보안·롤백·비용·모니터링 준비성이 필요하다. 이 항목을 Gate 6 안에 숨기면 QA 통과와 배포 승인 사이의 책임 경계가 흐려진다.

따라서 Release Readiness 는 다음 규칙을 따른다.

- Gate 6 PASS 또는 명시적 deferred decision 이후에만 실행한다.
- secret/auth/data/monitoring/rollback/cost 6개 체크박스를 최소 단위로 둔다.
- infra 본부가 abstract 인 Phase 1 minimal tier 에서는 별도 agent 호출 없이 human checklist 로 대체 가능하다.
- 실패해도 구현 PDCA 를 즉시 무효화하지 않는다. 배포만 보류하고 `release-blocker` 로 기록한다.

---

## 5.2 Gate별 Evaluator 매핑 (본부별)

| Gate | strategy-pm | taxonomy | design | dev | qa | infra |
|:---:|---|---|---|---|---|---|
| **Gate 1 (Intake)** | (공통, `discovery-report-validator` Sonnet — 본부별 축이 아니라 repo 전체 discovery 대상) | (공통) | (공통) | (공통) | (공통) | (공통) |
| **Gate 2 (Brainstorm)** | `intent-discovery-validator` + CPO review | taxonomy seed check | design opportunity check | tech feasibility sketch | risk framing check | infra/security risk sketch |
| **Gate 3 (Plan)** | `plan-validator` + `prd-validator` | `plan-validator` + `taxonomy-reqs-validator` | `plan-validator` + `design-reqs-validator` | `plan-validator` + `tech-reqs-validator` | `plan-validator` + `qa-reqs-validator` | `plan-validator` + `infra-reqs-validator` |
| **Gate 4 (Design)** | `user-flow-validator` | `taxonomy-draft-validator` | `design-critique` (cowork plugin 재사용) | `design-validator` (bkit 재사용) | `test-case-validator` | `terraform-validator` / `k8s-manifest-validator` |
| **Gate 5 (Handoff)** | `prd-lock-validator` | `taxonomy-consistency` | `accessibility-review` + `design-handoff` (cowork 재사용) | `code-analyzer` (bkit 재사용) | `qa-readiness` | `cost-estimator` + `security-scan` |
| **Gate 6 (Review)** | [§5.5 formula] | [§5.5 formula] | [§5.5 formula] | [§5.5 formula] | [§5.5 formula] | [§5.5 formula] |
| **Release Readiness** | release scope confirmation | naming freeze check | UX release notes check | deploy artifact check | QA evidence check | secret/rollback/monitoring/cost checklist |
| **Gate 7 (Retro)** | (공통, `sprint-retro-analyzer` Opus) | (공통) | (공통) | (공통) | (공통) | (공통) |

### 5.2.1 공통 Evaluator (`plan-validator`)

Gate 3에서 **모든 본부**가 공유. 다음 6가지를 확인:
...
| `gap-detector` | bkit | Gate 6 정량 축으로 사용 |
...
|-----------|------|------|--------|
| `discovery-report-validator` 🆕 | Gate 1 공통 — discovery-report 완성도 + 서명 블록 검증 | Sonnet 4.6 | P0 (brownfield 지원 시) |
| `plan-validator` | Gate 3 공통 | Sonnet 4.6 | P0 |
...
| `sprint-retro-analyzer` | Gate 7 학습 패턴 추출 | **Opus 4.6** | P0 |
...
gate_call:
  # 호출 식별
  call_id: "GC-042-gate5-001"        # 고유 ID (L1 logging key)
  sprint_id: "SP-005"
  pdca_id: "PDCA-042"
  gate_number: 5                     # 사용자 표기/리포트 기준
  gate_label: "Gate 5 (Handoff)"
  gate_id: "G3"                      # internal compatibility id for existing logs/files
  target_division: "dev"
  target_phase: "do"                 # plan | design | do | check | act

  # Evaluator 지정
  evaluator:
    agent_id: "qa/code-analyzer"
...
gate_report:
  # 호출 연결
  call_id: "GC-042-gate5-001"        # gate_call.call_id와 동일
  report_id: "GR-042-gate5-001"
  gate_number: 5
  gate_label: "Gate 5 (Handoff)"
  gate_id: "G3"                      # internal compatibility id
  schema_version: "v1"
  ...
  # 감사 (재현성)
  audit:
    evaluator_agent: "qa/code-analyzer"
...
- **비용 집계**: L3 driver view 의 "이번 Sprint 총 비용" 쿼리 소스
...
Gate 6은 **정량 축 40% + 정성 축(5-Axis CPO) 60%**의 가중합. 정량 축은 본부별로 다르다.

| 본부 | Gate 6 Formula | 정량 축 세부 | 비고 |
|------|-----------|-------------|------|
| **dev** | `Gate 6 = gap-detector × 0.4 + 5-Axis(CPO) × 0.6` | gap-detector = bkit 재사용, design ↔ code 일치도 | Gate 5 binary가 pre-check |
| **design** | `Gate 6 = design-guide-match × 0.4 + 5-Axis × 0.6` | design-guide-match = 디자인 시스템 토큰 준수율 | accessibility-review가 pre-check |
| **strategy-pm** | `Gate 6 = AC-coverage × 0.4 + 5-Axis × 0.6` | AC-coverage = PRD의 AC 중 Do 결과가 충족한 비율 | prd-lock이 pre-check |
| **taxonomy** | `Gate 6 = taxonomy-consistency × 0.4 + 5-Axis × 0.6` | taxonomy-consistency = 용어 충돌/중복 스캔 점수 | — |
| **qa** | `Gate 6 = test-coverage-delta × 0.3 + defect-leak × 0.1 + 5-Axis × 0.6` | 타 본부 상호검증 필요 없음 — qa의 산출물은 **타 본부의 Do 결과 자체를 검증한 리포트**이므로 메타-self-validation 문제 없음 | defect-leak = 테스트 후 프로덕션 버그 수 (역지표) |
| **infra** | `Gate 6 = cost-variance × 0.2 + stability-score × 0.2 + 5-Axis × 0.6` | cost-variance = 예산 대비 실집행, stability-score = 가용성/에러율 | cost-estimator + security-scan이 pre-check |

### 5.5.1 [OPEN → RESOLVED] 품질·인프라 본부 공식

이전 skeleton의 [OPEN] 항목 해결:

**품질 본부 Gate 6**: QA 산출물은 **타 본부의 Do 결과에 대한 테스트 리포트**이지, QA 자체가 만든 기능이 아니다. 따라서 self-validation 우려는 원칙적으로 없음 — QA evaluator가 평가하는 것은 "테스트 설계·실행의 완결성"이지 "자신이 만든 기능"이 아니다. 다만 두 가지 안전장치:
1. `test-coverage-delta`는 **다른 본부 code-analyzer**가 측정 (QA 본부 스스로 측정 X)
2. `defect-leak`은 Sprint+1 이후에만 확정 (시간차 검증)

**인프라 본부 Gate 6**: `cost-variance`(예산 ±10% 이내)와 `stability-score`(가용성 99% / 에러율 1% 이하)의 조합. 두 지표 모두 **외부 observability 시스템**(Datadog 등)에서 측정 — 인프라 agent 스스로가 측정하지 않음.
...
**CPO가 모든 본부 Gate 6에서 60% 가중치로 수행하는 정성 평가 5축**. 도메인 agnostic이고 Solon 수명 동안 고정.
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Gate 1 (Intake)** | ✅ | ✅ (report 재생성 ≤2회) | 🚨 abort install (Solon 미도입 결정) | 🚨 user-prompt (사람 서명 거부 누적 시) | — (단일 보고서) | ✅ | 🚨 repo 무결성 문제 시 |
| **Gate 2 (Brainstorm)** | ✅ | ✅ | 🚨 reject/re-scope Initiative | ✅ | ✅ | ✅ | 🚨 Initiative 보류 |
| **Gate 3 (Plan)** | ✅ | ✅ | 🚨 즉시 escalate | ✅ | ⚠️ 드물게 | ✅ | 🚨 Case-β |
| **Gate 4 (Design)** | ✅ | ✅ | 🚨 즉시 escalate | ✅ | ✅ | ✅ | ⚠️ Gate 3에서 잡혔어야 |
| **Gate 5 (Handoff)** | ✅ | ✅ | 🚨 즉시 escalate | 🚨 즉시 escalate | ✅ | ✅ | ⚠️ |
| **Gate 6 (Review)** | ✅ | ✅ | 🚨 즉시 escalate (Case-α) | ✅ | ✅ | ✅ | 🚨 Case-β |
| **Release Readiness** | ✅ | ✅ | 🚨 release-blocker | ✅ | ✅ | ✅ | 🚨 release 보류 |
| **Gate 7 (Retro)** | ✅ (정성) | — | ⚠️ Sprint 실패 → 재기획 | — | — | — | — |
...
- **Gate 1에서 CONFLICT은 논리적으로 불가**: Gate 1은 단일 discovery-report에 대한 단일 판정이므로 cross-division conflict 개념이 없음.
- **Gate 2의 FAIL-HARD = Initiative reject/re-scope**: 구현이 아니라 문제 정의가 부족한 상태이므로, PDCA로 밀어 넣지 않는다.
- **Gate 1의 STALL = 사용자 서명 거부 누적**: validator가 2회 이상 PASS 권고했는데도 사용자가 signature block에 서명하지 않으면 STALL → user-prompt로 "왜 보류하는가"를 되묻는다(원칙 10 작동).
- **Gate 1의 ABORT = Solon 미도입 결정**: repo 무결성 문제(예: shallow clone, detached HEAD, submodule 미초기화)로 discovery 자체가 불가능한 경우 → install abort.
- **Gate 5에서 STALL은 즉시 escalate**: Gate 5는 binary pre-check이므로 3회 반복된 FAIL은 구조적 문제 신호.
- **Gate 4의 ABORT은 이상신호**: Design 단계에서 요구사항 무효가 드러났다는 건 Gate 3 plan-validator가 놓쳤다는 뜻 → L3 대시보드에서 추적해 plan-validator 개선.
- **Gate 7은 정성 평가**: PASS/FAIL이 엄격한 의미가 아니라 "Sprint 회고에서 학습할 것이 있는가"의 표현.
- **Release Readiness 실패는 구현 실패가 아님**: release-blocker 로 기록하고 production deploy 만 보류한다.

### 5.8.2 Failure Mode 집계 메트릭 (§8 연결)

L3 driver view 에 다음 5개 핵심 지표로 표면화:
...
> **brownfield에서는 PLAN phase 이전에 P-1 Discovery phase + Gate 1 Intake가 먼저 실행된다.**
> ```
...
>   ├──> Gate 1 intake (discovery-report-validator + human signature)
...
> [greenfield는 여기부터 시작]
> ```
> 아래 greenfield 흐름은 Gate 1 PASS 이후에도 동일하게 실행된다.

```
[PLAN phase]
  │ plan document draft
  │ plan document finalize
  ├──> Gate 3 plan-validator (+ 본부별 reqs-validator)
  │       │
  │       ├─ SUCCESS ─┐
...
[DESIGN phase] <──────┘
  ...
  ├──> Gate 4 design-validator (+ 본부별 sub-validator)
  │       └─> SUCCESS 시에만 Do 진입
  │
[DO phase]
...
(Sprint 끝)
  │
[SPRINT RETRO]
  ├──> Gate 7 sprint-retro-analyzer (Opus)
  │       └─> next sprint plan 입력
```
...
| 구간 | 목표 | 비고 |
|------|------|------|
| Gate 1 validator 응답 | < 5분 | Sonnet × 2~3회 (schema 검증 + 완성도 scoring) |
| Gate 1 human review | 대기시간 제외 | repo 규모 Small 10~20분 / Medium 30~60분 / Large 2~4시간 (§7.10.8) |
| Gate 3 호출 → 응답 | < 2분 | Sonnet 단일 호출 + 병렬 sub |
| Gate 4 호출 → 응답 | < 5분 | design-critique + design-validator 병렬 |
| Gate 5 호출 → 응답 | < 3분 | binary check 위주 |
| Gate 6 호출 → 응답 | < 10분 | gap-detector(스캔) + CPO(Opus) 직렬 |
| Gate 7 호출 → 응답 | < 20분 | Sprint 전체 데이터 분석 |
...
|-------------|--------|-------|
| `ABORT` (Gate 1) | **install-abort** — Solon 미도입 결정 | `gate_report.escalation = {required: true, case: "install-abort"}` (§6에서 별도 루트) |
| `STALL` (Gate 1) | **user-prompt** (서명 보류 사유) | discovery-report는 PASS였는데 사용자가 signature 보류 |
| `FAIL-HARD` (Gate 3~6) | **Case-α** — AC-level reopen | `gate_report.escalation = {required: true, case: "alpha"}` |
| `ABORT` (Gate 3/Gate 6) | **Case-β** — 새 PDCA 생성 | `gate_report.escalation = {required: true, case: "beta"}` |
| `CONFLICT` (Gate 4~6) | **5-option protocol** | cross-division evaluator의 conflict 발견 |
| `STALL`/`TIMEOUT` (Gate 3~6) | **user-prompt** | Haiku가 user 개입 prompt 생성 |
...
3. Gate 7 시점에 `sprint-retro-analyzer`가 Sprint 전체 gate_report를 스캔
...
Gate 1은 구조가 Gate 3~7과 충분히 다르기 때문에 별도 섹션으로 격리한다. 본 섹션은 §4.3 (P-1 Discovery Phase), §7.10 (install.sh --mode brownfield), §2.10~2.12 (원칙 10·11·12)의 교차점이다.

### 5.11.1 Gate 1의 위치와 1회성
...
[Gate 1 Intake — 본 섹션]
    ...
    ├─ SUCCESS ─> P-1 종료 → Plan phase 진입 → Gate 2(brainstorm) or Gate 3(plan)
    ├─ FAIL-FIXABLE ─> discovery-report 재생성 (최대 2회)
    ├─ STALL ─> user-prompt ("왜 승인 보류하는가?")
    └─ ABORT ─> install 중단 + repo 원상복귀
```

**1회성 (per install)**: Gate 1은 Solon을 해당 repo에 도입할 때 **단 1번** 실행된다. 이후 Sprint가 몇 회 돌든, Initiative가 몇 개 추가되든 Gate 1은 다시 호출되지 않는다. 이는 `rule/p-1-run-once-per-install` (§4.3.10)과 일관된다.

### 5.11.2 Gate 1 Evaluator: `discovery-report-validator`

Gate 1의 core evaluator로 Phase 1에서 신규 구현.
...
| **FAIL-FIXABLE** | discovery-report validator가 지적한 필드 재생성 → Gate 1 재호출 (최대 2회) | 재생성 diff 로그 |
...
**중요**: Gate 1은 **`FAIL-HARD` / `CONFLICT` verdict를 생성하지 않는다**. discovery-report는 단일 문서이므로 cross-division conflict 개념이 없고, "설계 근본 문제"라는 판단은 brownfield intake 단계에서 시기상조(그건 Gate 3 이후 책임).
...
### 5.11.6 Gate 1과 Gate 2(Brainstorm)의 관계

Gate 1 PASS 후 곧바로 Gate 3(Plan)으로 가지 않고 **Gate 2(Brainstorm, §4 정의)** 를 거칠지 말지는 원칙 12로 제한된다:
...
| install 2년 후 **대규모 pivot** 논의 | ✅ | 제품 방향이 바뀌므로 Gate 2 필요 |

→ Gate 1은 "지금 이 repo를 Solon으로 운영할지"를 결정할 뿐, 기존 제품 방향은 **사전에 결정된 것**으로 간주한다 (원칙 12). 제품 방향 자체를 다시 brainstorm하고 싶다면 **새 Initiative**를 만들어 Gate 2를 명시적으로 호출해야 한다.

### 5.11.7 L1 Event Schema — `l1.gate.g-1.complete`

Gate 1이 verdict를 확정할 때 L1(S3 raw log)에 기록되는 event. 이벤트 명과 `gate_id` 값은 기존 로그 호환을 위해 유지한다.

> **주의**: 아래 JSON은 `l1-log-event-v1` base schema 위에 얹히는 **Gate 1 고유 필드**만 표기한다. 실제 L1 기록 시 base 필수 필드(`event_id`, `timestamp`, `session_id`, `agent_id`, `agent_role`, `model`, `tool_calls`, `input_tokens`, `output_tokens`, `latency_ms`, `trace_id`, `classification: "gate-execution"`, `gate_id: "G-1"`)가 함께 포함된다. base 스키마는 `appendix/schemas/l1-log-event.schema.yaml` 참조.
...
### 5.11.8 Gate 1 비용 예산 (Phase 1)

| 항목 | 비용 | 비고 |
|------|------|------|
| `discovery-report-validator` 2~3회 | $0.10 ~ $0.25 | Sonnet 4.6 |
| 기존 `/sfs discover` 비용 (§7.10.8) | Small <$2 / Medium <$15 / Large <$60 | 이는 Gate 1 이전 P-1 비용 |
| human review time | 10분 ~ 4시간 | 인건비 제외 시 $0 |
| **Gate 1 자체 추가 비용** | **< $0.30** | validator만 해당 |

→ Gate 1은 P-1 전체 비용의 **1% 미만**으로 설계되었다. 주 비용은 P-1 Discovery 단계이며, Gate 1은 그 결과를 검증하고 서명을 수집하는 **저비용 게이트**.

### 5.11.9 Gate 7과의 비교 (왜 Gate 1은 학습 루프에 들어가지 않는가)
...
→ Gate 1은 **도입 게이트**, Gate 7은 **학습 게이트**. 둘 다 "정성적" 요소가 있으나 기능적으로 완전히 다르다.

### 5.11.10 구현 순서 (Phase 1 체크리스트)

Phase 1에서 Gate 1을 지원하려면 다음 순서로 구현:
...
7. [ ] L3 대시보드 "Gate 1 pass rate" 패널 (선택, tier=collab 이상)
...
Release Readiness Gate 는 Gate 6 이후, production deploy 직전에 실행한다.

```
Gate 6 PASS or deferred decision
  └─> Dev Deploy + QA evidence
      └─> Release Readiness Gate
          ├─ SUCCESS → Production Deploy
          ├─ FAIL-FIXABLE → release-blocker 수정 후 재호출
          ├─ STALL/TIMEOUT → user-prompt
          └─ ABORT → release 보류, 구현 PDCA는 유지
```

이 gate 는 Gate 6을 대체하지 않는다. Gate 6은 "결과가 설계와 맞는가"이고, Release Readiness 는 "운영 환경에 올려도 되는가"다.

### 5.12.2 최소 체크리스트

Phase 1 minimal tier 에서는 agent 구현 없이 다음 6개 체크박스만으로도 gate evidence 를 남긴다.

| 체크 | 질문 | 실패 시 |
|---|---|---|
| secret | API key, DB password, token 이 코드/로그에 노출되지 않는가? | release-blocker |
| auth | 사용자/관리자 권한 경계가 명시됐는가? | release-blocker 또는 deferred |
| data | PII/결제/민감 데이터 저장·로그 정책이 있는가? | release-blocker |
| monitoring | error log, health check, alert 또는 최소 수동 확인 루트가 있는가? | deferred 가능 |
| rollback | 이전 버전 복구 또는 feature flag/off switch 가 있는가? | release-blocker |
| cost | 배포 비용/외부 API 비용 상한을 알고 있는가? | user decision |

### 5.12.3 GateReport 권장 형태

```yaml
gate_id: RELEASE
target: "release/<release-id>"
verdict: SUCCESS | FAIL-FIXABLE | STALL | TIMEOUT | ABORT
score: null
breakdown:
  secret: 1
  auth: 1
...
escalation:
  required: false
audit:
  evaluator: "infra/lead + human"
  started_at: "<ISO8601>"
  duration_ms: 0
user_decision_needed: false
```

`FAIL-HARD` 와 `CONFLICT` 는 기본 verdict 로 쓰지 않는다. Release Readiness 실패의 기본 의미는 "구현 무효"가 아니라 "배포 보류"이기 때문이다. 단, secret 유출처럼 구현 자체 수정이 필요한 사안은 `FAIL-FIXABLE` + `release-blocker` 로 기록하고 해당 PDCA 의 Act 또는 후속 patch PDCA 에 연결한다.

### 5.12.4 Phase 1 구현 방침

1. command 를 새로 늘리기보다 `/sfs check --release-readiness` 또는 `/sfs retro --pre-release` 형태로 흡수할지 결정한다.
2. schema 는 먼저 `gate_id: RELEASE` 를 허용한다.
3. 실제 CLI command 는 WU 확정 전까지 open question 으로 둔다.
4. 사용자 final approval 은 유지한다. Release Readiness PASS 는 production deploy 자동 실행을 의미하지 않는다.
