...
bkit에서는 Do 완료 → 바로 Check 게이트. 그러나 SFS는 **본부 간 핸드오프**가 잦기 때문에(디자인 → 개발 → QA), Do 단계 내에서 산출물이 "다음 본부가 받아먹을 수 있는 상태인지"를 **binary**로 먼저 자체 검증한다.
...
→ **기존 자산 재사용이 Phase 1 속도의 핵심**. SFS의 기여는 "언제/어떤 조합으로 호출할지"를 매트릭스로 정의한 것.
...
**CPO가 모든 본부 G4에서 60% 가중치로 수행하는 정성 평가 5축**. 도메인 agnostic이고 SFS 수명 동안 고정.
...
| 시스템 | 축 | SFS 5-Axis와의 차이 |
|--------|---|-----------------|
| RICE (Reach/Impact/Confidence/Effort) | 4축 | SFS는 **평가 도메인**(이미 만든 것)에 특화, RICE는 **우선순위** |
| OKR | Objective + Key Results | SFS 5-Axis는 **결과물 품질**, OKR은 **목표 설정** |
| DORA (lead time, deploy freq, MTTR, CFR) | 4축 | SFS는 **엔지니어링 메트릭이 아니라 결과물 자체**의 품질 |
| bkit `breakdown: {accuracy/completeness/style}` | 3축 | SFS는 **비즈니스 가치 축**(Value-Fit, User-Outcome)이 추가됨 |
