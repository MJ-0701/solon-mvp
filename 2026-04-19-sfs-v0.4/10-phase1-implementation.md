...
Phase 1은 **"SFS가 실제로 작동하는가?"** 를 자기 자신으로 증명하는 단계다.
기술 완성도보다 **dogfooding 증거**가 목적이다.

> 만약 Phase 1 종료 시점에 `product-image-studio` 프로젝트에
> SFS로 만들어진 Sprint Report + Gate Report + Escalation 1건 이상이 L2/L3에 남아있지 않다면,
...
| **Inspired-Rewrite (참조 후 독립 재작성)** | cowork plugin의 일부 skill | 아이디어는 참조하되 **코드는 SFS 내부 스타일로 새로 작성**. (§7.8) |
...
| bkit asset | SFS 내 역할 | 담당 Gate / 본부 |
|-----------|--------------|-----------------|
...
**재활용 시 주의**: bkit agent의 스코어 임계치는 SFS Gate 기준 (§5.5 G4 formula) 에 맞춰 매핑한다. 즉, bkit에서 Gap 90% = Pass지만, SFS Dev G4에서는 `Gap × 0.4 + 5-Axis × 0.6` 조합 중 Gap 성분만 담당한다.

### 10.3.3 cowork 재작성 (Inspired-Rewrite)

cowork plugin의 3개 skill만 SFS Design 본부로 재작성한다. **직접 의존성 금지** (§7.8 원칙).

| cowork skill (참조) | SFS 대응 Evaluator | 재작성 이유 |
|-------------------|-------------------|-------------|
| `design-critique` | `design-critique-rewrite` (SFS Design Evaluator) | cowork는 대화형 피드백 중심. SFS는 **Gate Report YAML** 로 구조화된 점수 출력이 필요. |
| `accessibility-review` | `accessibility-review-rewrite` | cowork는 WCAG AA 체크리스트 중심. SFS는 **Tier 2 외부 평가자**로서 Gate G4 점수 기여가 필요. |
| `design-handoff` | `design-handoff-rewrite` | cowork는 dev 팀 핸드오프 스펙 생성. SFS는 **Design → Dev 본부 핸드오프**로 변환, Gate G3 Pre-Handoff에서 소비. |
...
Phase 1 종료 시점에 **"Phase 2로 진입 가능한가"** 를 평가한다. 이 게이트 자체도 SFS의 G-series 확장 개념이지만, Phase 1의 최종 G5를 재활용한다.
