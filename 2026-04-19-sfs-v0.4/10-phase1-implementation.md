...
title: "§10. Phase 1 구현 계획 (16~20주, tier=minimal 기본 + brownfield optional)"
version: 0.4-r4
status: draft
...
references:
  ...
  - gate/g0-brainstorm (defined in: s05)
...
# §10. Phase 1 구현 계획 (16~20주, tier=minimal 기본 + brownfield optional)

> **Context Recap (자동 생성, 수정 금지)**
> §2~§9의 설계를 실제로 구현하는 16~20주 계획 (v0.4-r1의 14~18주에서 brownfield/activation/release-readiness 보강).
> Dogfooding 대상: `product-image-studio` (greenfield) + optional brownfield dogfooding 1건.
> Phase 1 성공 기준: active 본부 PDCA + 최소 1 abstract 본부 activation + G0/G1~G5 모두 1회 + production open 시 Release Readiness 체크 + Escalate 1회 + 3-Channel 데이터 보유.
>
> **v0.4-r2 변경**:
...
>
> **v0.4-r4 변경**:
> 1. 2026-04-28 startup team-agent 구조 리뷰 반영: Artifact Contract First / Taxonomy Is Root / Shift-left Infra-Security / Continuous Documentation Ledger 를 Phase 1 판정 기준에 연결.
> 2. Gate vocabulary 를 `G-1 + G0 + G1~G5 + RELEASE` 로 정합화. Release Readiness 는 command 가 아니라 production 전 checklist/gate vocabulary 로 먼저 둔다.
> 3. L3 default 를 `none` 으로 재확인. Notion 은 Phase 1 필수 backend 가 아니라 외부 공유가 필요할 때 켜는 driver 다.
...
- 10.4 16~20주 주차 breakdown (Foundation → Dogfooding + optional Brownfield)
...
| **Gate** | G-1 Intake (brownfield only) + G0 + G1~G5 모두 구현 + Release Readiness 체크 | §5 Gate Matrix |
...
| **3-Channel Observability** | L1 S3 + L2 git docs + L3 Human View (`none` default, notion optional) | §8.3 |
| **Tier Profile** | `minimal` 기본 구현 + `standard`·`collab` schema 정의만 | §7.4 |
| **L3 Backend Driver** | `none` (default) + `notion` (optional) 두 가지 구현; `obsidian`·`logseq`·`confluence`·`custom` schema만 | §8.4 |
...
**원칙**: Phase 1은 **"폭"보다 "깊이"**. 최소 기능으로 active 본부 PDCA + 최소 1개 abstract 본부 activation + G0/G1~G5 + H6 루프가 **end-to-end** 한 번 도는 것을 우선한다. Production open 이 있으면 Release Readiness 도 조건부로 포함한다. Brownfield 지원은 **"동작 증명"** 수준 (end-to-end P-1 + G-1 1회) 만으로 P0에 포함한다.
...
| `brownfield` | **P0 (must-ship)** — optional 검증 | P-1 + Plan/Design/Do/Check/Act | G-1 + G0/G1~G5 (+ RELEASE if production) | 위 + `/sfs discover` | §10.5.1 조건 1~6 + §10.5.1-b |
...
| **sfs-gates** | G-1 (brownfield) + G0/G1~G5 + RELEASE Gate 실행, GateReport 생성, Failure Mode 분기 | `/sfs-gate g-1 <install-id>`, `/sfs-gate g1 <feature>` | 공통 |
...
### 10.5.1 필수/조건부 조건 (greenfield 기준)
...
| 3 | G0 + G1~G5 Gate 모두 1회 이상 실행 | L1 S3 log에 `gate_id: G0/G1/G2/G3/G4/G5` 이벤트 존재 | Gate 프레임워크 검증 실패 |
| 4 | Escalate-Plan 1회 이상 트리거 + H6 학습 로그 1개 생성 | L2 escalation-plan.md 1건 + learnings-v1.md에 새 패턴 1건 | H6 자기학습 검증 실패 (H6 미검증) |
| 5 | 3-Channel observability 모두 데이터 보유 | S3 objects > 0, git docs 커밋 > 0, L3 driver=`none` 이면 local status/report 산출물 존재 또는 notion이면 대시보드 갱신 | H1 관측성 검증 실패 |
...
| **7** 🆕 R4 | **production 배포를 수반한 Sprint 에서 Release Readiness 체크 1회 이상 기록** | gate-report 또는 review/retro 산출물에 `gate_id: RELEASE` 상당 체크(secret/auth/data/monitoring/rollback/cost) 존재 | 배포 가능성 검증 실패. Phase 1 자체는 유지하되 production open 보류 |
...
> **결정 규칙**: 10.5.1의 7개 조건 중 1~6은 Phase 1 성공의 필수 AND. 7번 Release Readiness 는 production 배포가 있는 Sprint 에만 필수이며, 없으면 "not applicable" 로 기록한다. 10.5.1-b는 **Phase 1 성공 여부에 영향을 주지 않는 optional**. 단 §9.10 지표 (H5/H6 자기학습)에 brownfield 루프 데이터가 없으면 Phase 2 scope 결정 시 근거 부족 (§10.9 Transition 입력 감소). 따라서 **1건 수행을 강력 권장**.
...
| Metric | Target | 측정 소스 |
|--------|--------|----------|
| Sprint recovery time | < 24h | L3 driver view 또는 local report (첫 escalate → Sprint 재개) |
...
trigger: "Strategy-PM 본부는 '라이프스타일 이미지', Design 본부는 '배경 합성 이미지'로 동일 개념을 다른 이름 사용"
...
Phase 1 종료 시점에 **"Phase 2로 진입 가능한가"** 를 평가한다. 이 게이트 자체도 Solon의 G-series 확장 개념이지만, Phase 1의 최종 G5를 주 입력으로 사용한다. 실제 production open 을 수반하는 경우에는 §5.12 Release Readiness 를 별도 입력으로 붙인다.

### 10.9.1 Transition 판정 기준

| 조건 | 통과 기준 | 관련 §10.5 조건 |
|------|---------|----------------|
| Phase 1 필수 조건 1~6 모두 충족 | All PASS | §10.5.1 |
| Production open 포함 시 Release Readiness 통과 | PASS 또는 N/A | §10.5.1 조건 7 / §5.12 |
...
| **총계 (greenfield + brownfield)** | **16~20주** | — | **~410h (평균 주 21.6h)** |
...
*(끝 — §10 완료, Appendix command registry 는 13개 slash command + Release Readiness gate vocabulary 로 관리)*
