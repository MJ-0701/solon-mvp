...
reuse_count: 1   # 1회 첫 사례 + 25th-6 zen-magical-feynman W-22 자동화 적용 (cut-release.sh §1 pre-flight if-block + smoke 6건 PASS = exit code 6 신설)
related_patterns:
...
## §5 후속 자동화 후보 (W-22 TODO) — ✅ **resolved 25th-6 zen-magical-feynman**

- **WU-31.x refresh** 또는 별도 후속 WU 에서 cut-release.sh §1 pre-flight 보강:
  - `[[ -f "${STABLE_REPO}/.git/index.lock" ]] && abort_with_hint` ✅ **구현 완료**
  - `[[ -f "${DEV_REPO_ROOT}/.git/index.lock" ]] && warn` (dev side 충돌은 step 5 영향) ✅ **구현 완료**
  - stable working tree strict clean (`git -C stable status --porcelain --ignored | grep -q .` 류) — ⏭️ deferred (`--allow-dirty` 와 충돌 가능, 현 `check_clean()` 으로 충분)
- ~~처리 시점 = 26번째+ 사이클 사용자 결정 영역. β default = WU-31.x refresh (1줄 sed 패치 ~5분, cut-release.sh §1 함수 안 if-block 추가).~~
- **25th-6 zen-magical-feynman 적용** (Cowork conversation, 자율진행 batch): cut-release.sh §1 Pre-flight 함수 `check_index_lock()` 신설 (15L) + stable/dev 양쪽 호출 (stable=abort_in_apply=1 / dev=warn only) + Exit code 6 신설 ('stale `.git/index.lock` 검출, --apply 모드 한정'). 6 smoke PASS (sandbox `/tmp/wu31-w22-smoke-*/`, host 자산 mutate 0): T0 bash -n / T1 --help / T3a stable lock + --apply → exit 6 / T4a stable lock + dry-run → exit 0 (warn) / T5 lock 제거 후 정상 / T6 dev side lock + --apply → exit 0 (warn only). file 편집 2개 (cut-release.sh + 본 P-10) + 사용자 manual commit 완료 (commit `387a8d2`, 25th-6 batch squash, push origin main 완료). cross-ref-audit.md §4 W-22 status = resolved 동시 마킹.
- **25th-6 Codex review 후속**: dry-run 에서 stale lock 을 경고한 뒤 summary 가 `no stale .git/index.lock` 라고 말하던 문구 모순 수정 → `no blocking stale .git/index.lock`. 근거: dry-run 은 stale lock 을 허용하지만 --apply 를 막는 blocking stale lock 은 없다는 의미가 정확함.
