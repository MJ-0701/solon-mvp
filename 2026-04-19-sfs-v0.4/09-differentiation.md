...
> SFS의 차별화 축 6개. Phase 1에서 본격 차별화하는 것은 **H1 / H2 / H5 / H6**.
> H3 / H4는 Phase 2에서 본격화.
> 의사결정자(투자자, 공동창업자 후보)가 "왜 SFS인가"를 빠르게 파악할 수 있도록.
...
SFS의 차별화는 **축 6개**로 구조화된다. **Phase 1에서 본격화되는 것은 H1·H2·H5·H6** — 이 4개가 v0.4의 **의사결정자 어필 포인트**. H3·H4는 Phase 2로 연기.

| # | 축 | bkit / cowork / 범용 agent | SFS | Phase |
|:-:|---|-------------------------|-----|:---:|
...
`axis/h1-observability`: **같은 사실을 독자별 최적화된 세 view로 분리하는 것**이 SFS의 관측 전략.
...
| **SFS** | **L1 (S3) + L2 (git SSoT) + L3 (Notion)** | 독자별 최적화, SSoT 단일화 |

### 9.2.2 SFS의 관측 모델이 해결하는 것
...
- SFS: **모든 결정의 산출물(plan/design/gate/escalation)을 git에 표준 포맷으로 저장**, local cache(`.sfs-local/`)와 명확 분리
...
### 9.3.2 SFS의 6 본부가 채우는 공백

| SFS 본부 | 커버하는 역할 병목 (§0.2) | bkit/cowork로 못 채우는 이유 |
|---------|-----------------------|---------------------------|
...
| 범용 챗봇 | 사용자가 대화당 선택 |
| **SFS** | **역할별 할당 규칙** (C-Level→Opus, Lead/Worker→Sonnet, Prompt→Haiku), `config/models.yaml`로 override 가능 |

→ **모델 할당이 조직 설계의 일부** 라는 것이 SFS의 주장. "누가 얼마나 깊이 생각해야 하는가"를 비용으로 구현.
...
`axis/h6-self-learning`: **같은 실수를 두 번 반복하지 않게 하는 구조적 루프**. SFS가 시간이 지날수록 더 똑똑해진다는 주장의 엔진.
...
| **SFS H6** | ✅ | Escalation → G5 Retro → validator check 추가 → 다음 Sprint plan에 강제 적용 |

### 9.5.2 SFS H6 파이프라인 (§6.6 상세)
...
→ 이 3지표가 **SFS가 시간이 지나면서 똑똑해진다는 주장의 반증 가능한 근거**.
...
`axis/h4-production-ops`: SFS가 1인 창업 도구를 넘어 **소규모 팀 → 중견 기업**까지 쓰일 수 있게 하는 운영 기능.
...
- **Multi-tenant**: 1 SFS 인스턴스 → N 조직. 조직별 divisions.yaml / git repo / Notion DB 격리
...
> **SFS** = **1인 창업가의 회사 전체 운영 시스템** (6 도메인 × 3 C-Level × 3-Channel Observability × 자기학습 루프)

### 9.7.1 이 문장이 의미하는 것

- bkit은 **도구**, SFS는 **운영 시스템 (OS)**
- bkit의 scope는 **프로젝트**, SFS의 scope는 **회사**
- bkit은 **개발자**가 사용, SFS는 **1인 창업가 (개발자 포함)**가 사용
- bkit은 **제품 PDCA**, SFS는 **제품 × 도메인 × 학습의 Sprint**

### 9.7.2 포지션 검증 질문

투자자·공동창업자 후보·초기 사용자가 흔히 물을 질문과 SFS의 답:

| 질문 | 답변 (한 줄) |
|------|---|
| "bkit 쓰면 되는 거 아니에요?" | bkit은 개발 PDCA만. SFS는 기획~인프라까지 6 도메인. |
| "LangChain 에이전트 프레임워크로 만들면?" | LangChain은 실행 엔진. SFS는 **조직 설계**. 둘은 다른 레이어. |
| "AutoGPT 변형 아닌가?" | AutoGPT는 loop. SFS는 **외부 검증 게이트 + 학습**. 자기검증 금지(원칙 2.2)가 정반대. |
...
| **SFS** | ✅ | ✅ | ✅ P1 기반 / P2 상품화 | ✅ P2 | ✅ | ✅ | **회사 운영 OS** |

### 9.8.1 매트릭스 해석

- SFS가 6축 모두 ✅인 **유일** 도구. 다만 H3·H4는 Phase 2 본격화 — Phase 1은 H1/H2/H5/H6에 집중.
- bkit이 개발 PDCA에서 가장 가까운 상대. SFS는 bkit을 **재사용**하되 위에 5축(H2 외 전부)을 얹는다.
- LangChain/AutoGPT/Devin류는 다른 레이어(실행 엔진). SFS는 그 위에서 **조직 설계**를 한다. 충돌 아니라 **상보적**.

### 9.8.2 매트릭스가 말하지 않는 것

- **완성도**: bkit은 성숙, SFS는 설계 단계. 완성도 축은 시간 문제.
- **학습 곡선**: SFS는 6 본부/5 Phase/G1~G5 등 **개념 밀도**가 높음. bkit이 훨씬 배우기 쉬움.
- **비용**: SFS는 Opus 사용 빈도가 높아 **월 운영비가 비쌈**. bkit/cowork보다 비쌀 수 있음.

→ SFS는 "더 많은 것을 하지만 더 무겁다". **모두의 첫 도구는 아니다** — 1인 창업가가 실제로 7 역할 병목을 느낄 때의 도구.
...
- SFS: **다른 세션, 다른 모델, 다른 파일 권한, 다른 read-only 플래그** — 구조적으로 분리

→ role play는 prompt, SFS는 runtime.

### 9.9.3 "PDCA 템플릿 모음 아냐?"

bkit의 PDCA 템플릿(docs/standards/pdca/)과 SFS의 차이:
- bkit 템플릿: plan/design/analysis/report의 markdown 양식
- SFS: **템플릿 + Gate + Escalate + 학습 루프 + 3-Channel sync + 6 본부** 의 총합

→ 템플릿은 SFS의 **일부 (appendix/templates/)**, 전부가 아님.

### 9.9.4 "프롬프트 엔지니어링 체계 아냐?"

- 프롬프트 엔지니어링: agent 1개에 더 좋은 프롬프트를 주는 것
- SFS: **프롬프트 간 관계 + 산출물 스키마 + 검증 루프**를 구조화

→ 프롬프트는 worker agent의 일부, SFS의 core는 **워크플로우 구조**.
...
- 5개 모두 달성 → **SFS의 주장이 증거 기반으로 뒷받침됨** → Phase 2 투자 근거
- 1~2개 미달 → 해당 축 재설계 후 Phase 1 연장
- 3개 이상 미달 → SFS 설계 자체를 재검토 (구조적 결함 가능성)
