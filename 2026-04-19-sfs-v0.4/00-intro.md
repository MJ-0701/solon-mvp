...
> **이 섹션은 채명정 본인이 직접 선언한 Solon 제품 철학이다. 두 선언이 전체 설계의 근거이며, 각각 [원칙 9](02-design-principles.md)·[원칙 10](02-design-principles.md)으로 구조화된다.**
...
```
[사람: 의도 선언] → G0 Brainstorm → [agent 병렬: 실행] → G1~G4 Gate → RELEASE → [사람: 최종 판정]
   ↑ 철학 ① 작동                                                                 ↑ 철학 ② 작동
       (진입 필터)                                                        (출구 필터)
```
...
- 0.4 Solon의 핵심 주장 (Company-as-Code)
- 0.5 이 문서셋을 어떻게 읽을지
...
2. **프로세스** — Initiative ⊃ Sprint ⊃ PDCA + **G-1/G0/G1~G5/RELEASE Gate vocabulary** + Escalate-Plan
3. **산출물** — 본부별 표준 산출물 + Artifact Contract + schema (gate-report-v1, escalation-v1, L1 log-event)
4. **관측성** — 3-Channel (L1 events / L2 git SSoT / L3 driver, default none) 일방향 sync
...
Solon은 이 공통 빈 칸을 **구조**로 메운다. (자세한 축별 비교는 [§9](09-differentiation.md))
...
- 프로세스 → PDCA + G-1/G0/G1~G5/RELEASE Gate vocabulary + Escalate-Plan 규칙
