...
7. **회고 + 보고서 압축** (G5) — `retro.md` 로 history/learning 을 남기고 `report.md` 로 최종 작업보고서를 만든 뒤 workbench 문서를 compact
...
`/sfs retro --close` 는 workbench 문서들을 작은 redirect/stub 로 압축한다. 히스토리와 학습은
`retro.md`, session log, `.sfs-local/events.jsonl` 이 담당한다.
...
Solon SFS 의 13 명령은 어떤 CLI 환경에서 호출하든 동일한 `sfs` runtime 이 SSoT 다 (paraphrase 금지):
...
| `/sfs report [--sprint <id>] [--compact]` | 최종 `report.md` 생성/갱신. `--compact` 는 사용자 동의 후 workbench 문서를 redirect/stub 로 압축 |
| `/sfs retro [--close]` | retro.md 작성/갱신. `--close` 는 retro + report refinement 후 workbench compact + sprint close + auto commit |
