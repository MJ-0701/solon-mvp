...
| `/sfs commit plan` | close 후 남은 working tree 를 product-code / sprint-meta / runtime-upgrade 로 분류 |
| `/sfs commit apply --group product-code -m "feat: ..."` | 선택 그룹만 local commit. untracked/unstaged 포함, push 안 함 |
...
`/sfs retro --close` 는 sprint close bookkeeping commit 을 만든다. 코드 본체나 runtime upgrade 처럼 close commit 에 들어가지 않은 변경은 `/sfs commit plan` 으로 그룹을 확인한 뒤 `/sfs commit apply --group ...` 로 별도 local commit 한다. **`git push` 는 절대 자동 안 함** — 모든 vendor / Skill / slash 가 동일.
