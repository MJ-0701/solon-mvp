...
Solon 버전 갱신 후에는 uninstall/reinstall 대신 `sfs update` 를 실행한다.
...
`/sfs implement` 로 코드와 evidence 가 생긴 뒤 CPO review 를 연다. Claude 로 구현했다면 CPO review 는
...
| `/sfs commit apply --group product-code` | 선택 그룹만 local commit. Git Flow-aware Conventional Commit 메시지 자동 생성, `-m` override 가능, push 안 함 |
...
`/sfs retro --close` 는 sprint close bookkeeping commit 을 만든다. 코드 본체나 runtime upgrade 처럼 close commit 에 들어가지 않은 변경은 `/sfs commit plan` 으로 그룹과 branch preflight 를 확인한 뒤 `/sfs commit apply --group ...` 로 별도 local commit 한다. 메시지는 Git Flow-aware Conventional Commit 으로 자동 생성된다. branch 생성/switch helper 는 placeholder 안내만 제공하며 아직 자동 실행하지 않는다. **`git push` 는 절대 자동 안 함** — 모든 vendor / Skill / slash 가 동일.
...
### 8.1 `sfs guide/status/update` 가 "project is not initialized" 출력
...
아니. 프로젝트 루트에서 `sfs update` 한 번만 실행한다. Homebrew 설치본이면 먼저
global runtime 을 최신화한 뒤 현재 프로젝트 adapter/docs 를 갱신한다.
