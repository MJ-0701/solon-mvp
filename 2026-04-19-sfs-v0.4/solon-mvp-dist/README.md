...
7. `/sfs status` 와 `.sfs-local/events.jsonl` 로 현재 상태를 확인합니다.
8. 필요하면 Codex/Gemini/Claude 가 같은 문서 계약을 읽고 다음 작업을 이어갑니다.
...
| `/sfs retro --close` | AI runtime 에서는 retro 작성 후, review 실행 여부 확인 + sprint close + auto commit. 이후 branch push/main 흡수는 Git Flow lifecycle 로 처리 |
...
설치 후 12 명령은 **세 런타임 모두에서 1급 entry point** 로 연결됩니다.
