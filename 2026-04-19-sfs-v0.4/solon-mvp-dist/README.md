...
- `start` 는 새 작업 공간을 만들고, 새 요구 탐색에는 brainstorm 방향 선택지를 보여줍니다.
- `brainstorm` 은 요구사항을 바로 계획으로 굳히지 않고 의도, 우선순위, 포기할 것, 성공 기준을 묻습니다.
- `plan` 은 대화록이 아니라 목표, 범위, 완료 기준, 확인 방법을 담는 짧은 계약입니다.
- `implement` 는 코드만 뜻하지 않습니다. 문서, 전략, 디자인 handoff, QA evidence, 운영/runbook 도 산출물입니다.
- `review` 는 코드리뷰 하나가 아니라 산출물 검토입니다. Solon 이 code/docs/strategy/design/QA/ops/release 흐름을 보고 자동 추론합니다.
- `retro` 는 일반적인 sprint 마무리 명령입니다. report 정리, archive, close 를 함께 처리합니다.
...
SFS 는 token/harness hygiene 도 일반 명령 흐름 안에 조용히 적용합니다. Agent 는 adapter 문서를
얇게 유지하고, 필요한 context 만 읽고, 큰 코드베이스에서는 symbol/semantic search 를 우선하도록
안내받습니다. 반복 실수는 다음에도 말로 설명할 항목이 아니라 check/hook/안전장치로 바꿉니다.
터미널에서는 adapter docs 나 sprint workbench 가 비대하거나 큰 코드베이스가 감지될 때 하루 1회
이하로 hygiene notice 를 띄웁니다. 끄려면 `SFS_HYGIENE_NOTICE=0` 을 사용합니다.
...
- 일반적인 마무리는 `sfs retro` 입니다. `report` 와 `tidy` 는 필요할 때만 따로 씁니다.
- sprint 중 생긴 긴 workbench 는 완료 시 `report.md` / `retro.md` 로 압축하고 archive 합니다.
