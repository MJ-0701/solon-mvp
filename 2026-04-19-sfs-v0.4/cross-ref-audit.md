...
22. **cut-release.sh §1 pre-flight 보강 (stable + dev `.git/index.lock` + stable working tree strict clean)** — 25th-2 affectionate-laughing-pascal 옵션 β 0.3.0-mvp 첫 실 release cut 시 step 4 의 stable rsync + VERSION + CHANGELOG.md prepend 정상 진행 후 `git -C stable add -A` 단계에서 `fatal: Unable to create '<stable>/.git/index.lock': File exists` abort. 원인 = stable repo (`~/workspace/solon-mvp`) `.git/index.lock` stale 잔존 (이전 git process abort 흔적, 다른 git process 0). 복구 = manual 4-step (rm lock + manual git add/commit/tag + manual stable push + manual dev post-flight). 사고 narrative = `learning-logs/2026-05/P-10-stable-stale-git-lock-recovery.md`. 처리 시점 = (A) WU-31.x refresh (cut-release.sh §1 pre-flight if-block 추가 ~5분, β default) / (B) 별도 후속 WU 신설 (P-07 phase 승급 검증 후 WU-32+ 와 묶음) / (C) GitHub Action drift 알림 (WU-32 spec) 에 release cut 사고 detection rule 추가 — 26번째+ 사이클 사용자 결정 영역. P-07 (release-tooling-phased) reuse_count +1 = 본 사고가 phase 승급 invariant 의 실 가치 입증.
