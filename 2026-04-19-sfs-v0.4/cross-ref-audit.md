...
22. **cut-release.sh §1 pre-flight 보강 (stable + dev `.git/index.lock` + stable working tree strict clean)** — ✅ **resolved 25th-6 zen-magical-feynman (Cowork conversation, 2026-04-29)**. 25th-2 affectionate-laughing-pascal 옵션 β 0.3.0-mvp 첫 실 release cut 시 step 4 의 stable rsync + VERSION + CHANGELOG.md prepend 정상 진행 후 `git -C stable add -A` 단계에서 `fatal: Unable to create '<stable>/.git/index.lock': File exists` abort. 원인 = stable repo (`~/workspace/solon-mvp`) `.git/index.lock` stale 잔존 (이전 git process abort 흔적, 다른 git process 0). 복구 = manual 4-step (rm lock + manual git add/commit/tag + manual stable push + manual dev post-flight). 사고 narrative = `learning-logs/2026-05/P-10-stable-stale-git-lock-recovery.md`. ~~처리 시점 = (A) WU-31.x refresh (cut-release.sh §1 pre-flight if-block 추가 ~5분, β default)~~ **25th-6 옵션 (A) 채택**: cut-release.sh §1 Pre-flight 에 `check_index_lock()` helper 함수 (15L) + stable abort (exit 6) / dev warn only 호출 + Exit code 6 신설 ('stale `.git/index.lock` 검출, --apply 모드 한정'). 6 smoke PASS (T0 bash -n / T1 --help / T3a stable lock + --apply → exit 6 / T4a stable lock + dry-run → warn / T5 lock 제거 후 정상 / T6 dev side lock + --apply → warn). file 편집 3개 (cut-release.sh + P-10.md + 본 cross-ref-audit) + 사용자 manual commit 완료 (commit `387a8d2`, 25th-6 batch squash, push origin main 완료). P-10 reuse_count 0→1. P-07 (release-tooling-phased) reuse_count +1 = 본 사고가 phase 승급 invariant 의 실 가치 입증.

**⚠️ 추가 W10 결정 (25th-5 발견, 2026-04-29, zen-practical-archimedes)** — `auto-resume.sh` HANDOFF 우선 규칙 미명시:

23. **auto-resume.sh HANDOFF 우선 규칙 (W-23, 신선도 비교 + `--prefer-handoff` flag)** — ✅ **resolved 25th-6 zen-magical-feynman (Cowork conversation, 2026-04-29)**. 25th-5 zen-practical-archimedes (Cowork sandbox) round-trip dogfooding 시 `auto-resume.sh` v0.1 가 stale `resume_hint.default_action` (0.5.0-mvp 시점 작성) 만 출력 → 신선 `HANDOFF.md` (0.4.0-mvp release cut 직후 작성, written_at 더 최신) 우선 규칙 부재 = AI 가 잘못된 default 따라갈 위험. 처리 = (A) `auto-resume.sh` v0.2 `--auto` (default) 신선도 자동 비교 + `--prefer-handoff` / `--prefer-progress` 강제 override flag + JSON 출력에 `source` / `prefer_mode` / `progress_last_written` / `handoff_written_at` / `progress_default_action` / `handoff_default_action` 필드 추가 / (B) `CLAUDE.md §1.19` 1줄 보강 ('신선도 우선 규칙' 명시) / (C) version 1.18→1.19 bump. 11 smoke PASS (T0~T11): T0 bash -n / T1 --help / T2 --auto HANDOFF newer (07:50) > PROGRESS (07:35) → source=handoff (사용자 발견 사례 핵심) / T3 --prefer-handoff explicit / T4 --prefer-progress (v0.1 backwards-compat) / T5 HANDOFF missing → progress fallback / T6 PROGRESS missing → exit 2 / T7 --prefer-handoff + HANDOFF missing → graceful fallback / T8 둘 다 비어 있음 → exit 3 / T9 --format text / T10 HANDOFF older than PROGRESS → source=progress / T11 unknown flag → exit 1. file 편집 3개 (auto-resume.sh + CLAUDE.md + 본 cross-ref-audit) + 사용자 manual commit 완료 (commit `387a8d2`, 25th-6 batch squash, push origin main 완료). 다음 26번째 + 후속 세션 진입 시 자동 적용 = 본 W-23 자체가 첫 실 가치 검증 case.

24. **Live LLM CLI shape (W-24, WU27-D6)** — ⏳ **deferred 26th-1 admiring-compassionate-euler (Cowork user-active-deferred, 2026-04-29)** sub-task 6.8.2 audit 발견 + 사용자 D′ 결정. `review_with_persona` 의 `SFS_LOOP_LLM_LIVE=1` 모드는 본 cycle 에서 **fail-closed (rc=99 + verdict=ERROR)** 로 명시 — 실 LLM CLI 호출 = **claude / gemini / codex 별 stdin 처리 / flag 명세 / exit code parsing 모두 다름** = 추측 구현 시 후속 단계에서 더 비싼 cost 부담. 결정 영역 = (a) 어떤 executor 들을 first-class support / (b) persona text + plan_doc 을 어떻게 stdin 으로 합쳐 호출 / (c) verdict prefix parsing 룰 (verdict: PASS|FAIL|PASS-with-conditions 첫 줄 매칭?) / (d) timeout cap (~1-2분/호출 §9 후속 TODO). 처리 시점 = 0.5.0+ release cycle, 외부 onboarding 1~2건 후 실 사용자 환경에서 어떤 executor 가 우세한지 데이터 누적 후. 본 cycle 한정 임시 mitigation = `_builtin_persona_text()` helper 신설 (planner/evaluator 4-line built-in fallback) + persona file 부재 시 known-name → fallback / unknown-name → rc=99 (W-25 와 동시 도입, 26th-1 사용자 추가 정책).

25. **PROGRESS.md domain_locks schema migration policy (W-25, WU27-D7)** — ⏳ **deferred 26th-1 admiring-compassionate-euler (Cowork user-active-deferred, 2026-04-29)** sub-task 6.8.4 audit 발견 + 사용자 D′ 결정. CLAUDE.md §1.16 FSM 4-state schema (`status: PROGRESS | COMPLETE | FAIL | ABANDONED` + `version: <int>` + `retry_count: <int>` + `failed_at` + `fail_reason`) 는 현재 `claim_lock` 첫 호출 시 lazy `repl_or_add` 로 자동 inject 되도록 코딩됨. 결정 영역 = (a) 기존 7개 도메인 (D-A~D-I) 에 일괄 migration vs lazy inject 유지 / (b) migration 시점 = 0.5.0-mvp release cut 전 vs cut 후 / (c) backwards-compat = §1.16 추가 전 schema 와 추후 schema 의 mixed state 운영 정책 (현재 실 데이터에서 stale FAIL detect 한 케이스 0건 = 즉시 migration 의 가치 미검증). 본 cycle 한정 = lazy inject 유지 (사용자 명시 결정 = "전역 schema migration 은 하지 않는다 / 운영 정책 변화라 후속 W10 로 남긴다 / 이번에는 lazy inject 유지"). 후속 검증 신호 = stale FAIL 또는 ABANDONED transition 1회 이상 발견 + 외부 onboarding 후 mixed schema operational 영향 측정.

26. **단계적 안내 원칙 신설 (W-26, CLAUDE.md §1.20)** — ✅ **resolved 26th-1 admiring-compassionate-euler (Cowork user-active-deferred, 2026-04-29)** 사용자 verbatim 비판 + 즉시 등재. 본 cycle ε 후속 (sub-task 6.8 buffer 종결 + handoff cycle 전환 시점) 에서 "진단 + 옵션 + 다음 단계 batch dump" 패턴 2회 dogfooding 후 사용자 비판 ("진단도 단계가 여러개인데, 하나에서 문제가 발견되면 그걸 수정을 먼저 해야되는데 이때는 단계적으로 하나씩 절차대로 진행해야되는데 지금 쓸데없이 옵션이나 다음단계가 같이 튀어나옴 필요한 부분만 → 정상흐름일땐 다음단계 안내까지만"). 처리 = (A) `CLAUDE.md §1.20` 신설 (172L, ≤200L cap 안) — 정상 흐름 = 다음 1 step 만 / 문제 발견 = 즉시 stop + 그 문제만 / 옵션 분기 = 결정 갈림길에서만. §1.17 (7-step briefing prose) 의 cousin = "절차 안내" 영역. (B) `learning-logs/2026-05/P-11-sequential-disclosure.md` 신설 — bad case 2건 + good case 1건 worked example. (C) `CLAUDE.md` version 1.19 → 1.20 bump. 다음 cycle 진입 시 본 §1.20 자동 적용 + dogfooding 누적.

**⚠️ 추가 W10 결정 (26th-3 발견, 2026-04-30, ε continuation 3 0.5.0-mvp 회귀 사고 + 사용자 비판 2건)**:

27. **dev-stable narrative divergence on cut (W-27, P-13 + cut-release.sh §1 강화)** — ✅ **resolved 26th-3 ε continuation 3 (Cowork user-active conversation, 2026-04-30)**. 26th-2 helper 0.5.0-mvp release cut (`99b2313`) 가 dev staging 의 mvp 본을 stable 에 rsync 하면서 codex 의 product positioning narrative 3 commits (`ced9cc1` + `5765abb` + `7977a75`) 을 overwrite. 외부 onboarding 차단 위험 (curl URL `MJ-0701/solon-mvp` GitHub redirect 만료 시 404). 처리 = (A) `cut-release.sh §1 Pre-flight` 강화 — stable HEAD 의 narrative key (VERSION suffix / README h1 / CHANGELOG h1 / SOLON_REPO 상수) 와 dev staging 비교 → divergence 감지 + warn (dry-run) / abort (--apply, exit 7 신설) + `--allow-divergence` bypass flag. git log 비교 (last release tag 이후 stable 에 sync-back 없는 commit) 도 추가. (B) `learning-logs/2026-05/P-13-dev-stable-divergence-on-cut.md` 신설 — 사고 narrative + detect / prevent / recover 3 path. (C) `CLAUDE.md §1.13` 보강 — sync-back 단계 누락 시 다음 cut 회귀 risk 명시 + cut-release.sh divergence 감지 link + version 1.20 → 1.21 bump. 다음 release cycle 시 stable 에 codex hotfix 가 있으면 abort 되는지 1~2 사이클 검증.

28. **AI mental coupling on rename / refactor fix (W-28, P-14 + R-D1 §1.13 보강)** — ✅ **resolved 26th-3 ε continuation 3 후반 (Cowork user-active conversation, 2026-04-30)**. 26th-3 첫 시도 가 codex 의 stable rebrand 작업 narrative 11항목 (README product positioning rewrite / public terminology cleanup / `/sfs start <goal>` contract / runtime asset sync / non-TTY handling / decision JSONL escape / distribution hygiene / artifact contract docs / maintenance history contract / repository rename 등) 을 보존 안 하고 단순 string replace (`solon-mvp` → `solon-product`, h1 변경) 만 함. 사용자 verbatim 비판 = "코덱스 수정사항이 합리적이면 그걸 반영하란 소린데... 너 그냥 무작정 회귀만했지". 처리 = (A) 즉시 stop + sync-back path 정합 진행 — `git show 7977a75:README.md` 로 codex 의 final README 추출 (product positioning 구조 = Why Solon / How It Works / Operating Model / Quickstart / Product Surface / Runtime Coverage / Installation / Installed Files / Safety Contract / Repository Map / Release Channel) + 본 cycle multi-adaptor 1급 (4 entry point + 7-Gate enum + Codex Skills + Gemini commands) 통합으로 dev staging README overwrite. CHANGELOG entry 도 codex 0.4.1-mvp Unreleased 의 11항목 narrative + 본 cycle 변경 통합으로 재작성. helper commit message narrative 도 sync-back 본질 정합. (B) `learning-logs/2026-05/P-14-mental-coupling-on-rename-fix.md` 신설 — invariant = "stable hotfix 발견 시 git show 로 narrative read 의무 후 sync-back" + 5단계 체크리스트 (Read / Investigate / Classify / Decide / Verify) + 본 cycle dogfooding worked example (bad case + good case). (C) `CLAUDE.md §1.13` 보강 — "단순 string replace 가 아니라 git show 로 narrative read 후 통합 — P-14 mental coupling 안티패턴 차단" 명시. 본 cycle 자체가 P-14 첫 dogfooding case = 사용자 비판 즉시 수신 → 자가 인지 → sync-back 정합 path 채택 = good case 정합 회복.

29. **Codex CLI `/sfs` compatibility gap (W-29, WU-34)** — ⏳ **open 2026-04-30 Codex desktop/user-active follow-up**. 확인된 사실: Codex desktop app / compatible Codex surface 는 `/sfs ...` 를 prompt 본문으로 모델/Skill 에 전달할 수 있으므로 0.5.5-product 에서 canonical 1급 경로로 복구 완료. 반면 codex-cli blocking build 는 `/sfs status` 입력을 native slash parser 가 모델/Skill 전에 차단하며 `Unrecognized command '/sfs'. Type "/" for a list of supported commands.` 를 반환한다. 따라서 `.agents/skills/sfs/SKILL.md` 는 메시지를 받지 못하고 dispatch 할 수 없다. `$sfs ...`, `sfs ...`, 자연어, direct bash 는 임시 bypass 일 뿐 parity 완료가 아니다. 추적 WU = `sprints/WU-34.md`. 처리 원칙 = desktop/app `/sfs` 제거/격하 금지, CLI bare `/sfs` 는 공식 등록면 또는 upstream 지원 신호 전까지 compatibility gap 으로 유지, 다음 micro-step 은 Windows wrapper 실기기 확인 후 codex-cli version/failure transcript backfill.
