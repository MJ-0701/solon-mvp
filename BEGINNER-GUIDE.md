...
성공하면 `sfs 0.6.52`, `status up-to-date` 같은 문장이 보입니다. Windows
...
0.6.52 기준으로는 Windows PowerShell/cmd 안내와 smoke 를 `sfs.cmd` 경로로 고정합니다.
...
그 값이 비어 있으면 `CMDCMDLINE` 원본 명령행을 fallback 으로 읽습니다. `sfs.ps1` 이
