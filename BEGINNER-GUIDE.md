...
성공하면 `sfs 0.6.47`, `status up-to-date` 같은 문장이 보입니다. Windows
...
0.6.47 기준으로는 Scoop 이 만든 shim 이 packaged `bin\sfs.ps1` 을 직접 실행합니다. `sfs.cmd` 는
직접 실행/호환용 thin PowerShell trampoline 으로 남고, 받은 인자를
