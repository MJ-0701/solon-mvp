...
assert_contains "${release_context}" "배포해줘" "release context Korean deploy trigger"
assert_contains "${release_context}" "배포 프로세스 쭉 진행해줘" "release context Korean full process"
assert_contains "${release_context}" "review/검수" "release context review before done"
assert_contains "${release_context}" "installed runtime verification" "release context installed runtime verification"
assert_contains "${release_context}" "not a publish-only command" "release context not publish only"
...
assert_contains "${readme}" "배포해줘" "readme Korean deploy trigger"
assert_contains "${readme}" "배포 프로세스 쭉 진행해줘" "readme Korean full process"
assert_contains "${readme}" "review/검수" "readme review before done"
assert_contains "${readme}" "설치 runtime 검증" "readme installed runtime verification"
