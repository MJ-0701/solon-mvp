...
assert_contains "${implement}" "If an external GitHub/@codex/PR/check PASS arrives before self-CPO or" "implement external pass continuation"
assert_contains "${implement}" "configured Codex/Claude/Gemini cross-review order after self-CPO PASS" "implement cross order"
...
for file in "${adapter_files[@]}"; do
  ...
  assert_contains "${file}" "sfs review --sprint <id> --gate <n>" "adapter closed sprint command ${file}"
  assert_contains "${file}" "cross review after self-CPO PASS" "adapter cross after self CPO ${file}"
done
