...
[[ "${plain_output}" == "sfs 0.8.12" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.12" "version output"
assert_contains_text "${output}" "latest 0.8.12" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline 하네스 레퍼런스와 LLM Wiki 지식 냉장고 관점" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 하네스 레퍼런스와 LLM Wiki 지식 냉장고 관점" "release notes fallback headline"
