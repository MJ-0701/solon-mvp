...
[[ "${plain_output}" == "sfs 0.8.13" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.13" "version output"
assert_contains_text "${output}" "latest 0.8.13" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline 자율주행 루프를 모델 진화" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 자율주행 루프를 모델 진화" "release notes fallback headline"
