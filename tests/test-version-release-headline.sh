...
[[ "${plain_output}" == "sfs 0.8.64" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.64" "version output"
assert_contains_text "${output}" "latest 0.8.64" "latest output"
...
assert_contains_text "${output}" "usage-value signal" "0.8.64 headline opening clause"
...
assert_contains_text "${fallback_output}" "사용량=가치 신호" "0.8.64 release-notes distinctive clause"
