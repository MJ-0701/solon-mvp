...
[[ "${plain_output}" == "sfs 0.8.9" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.9" "version output"
assert_contains_text "${output}" "latest 0.8.9" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline 강의 레퍼런스 인사이트를 SFS 검토 렌즈" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 강의 레퍼런스에서 뽑은 AI-era 운영 렌즈" "release notes fallback headline"
