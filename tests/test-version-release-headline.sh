...
[[ "${plain_output}" == "sfs 0.8.20" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.20" "version output"
assert_contains_text "${output}" "latest 0.8.20" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline Review budget guardrails, measure dashboard, lite first experience, and richer HTML artifacts ship together." "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline Review budget guardrails, measure dashboard, lite first experience, and richer HTML artifacts ship together." "release notes fallback headline"
