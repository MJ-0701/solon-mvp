...
[[ "${plain_output}" == "sfs 0.7.3" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.7.3" "version output"
assert_contains_text "${output}" "latest 0.7.3" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline Consumer-side AS surface" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 이번 버전은 0.7.2 doc-separation" "release notes fallback headline"
