...
[[ "${plain_output}" == "sfs 0.9.0" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.9.0" "version output"
assert_contains_text "${output}" "latest 0.9.0" "latest output"
...
assert_contains_text "${output}" "excavation pipeline" "0.9.0 headline opening clause"
...
assert_contains_text "${fallback_output}" "역추적" "0.9.0 release-notes distinctive clause"
