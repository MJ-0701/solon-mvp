...
[[ "${plain_output}" == "sfs 0.12.2" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.12.2" "version output"
assert_contains_text "${output}" "latest 0.12.2" "latest output"
...
assert_contains_text "${output}" "A derived-doc insight sharpens the provenance rail" "0.12.2 headline opening clause"
...
assert_contains_text "${fallback_output}" "파생 문서 주석 보존 규율" "0.12.2 release-notes distinctive clause"
