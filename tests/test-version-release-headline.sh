...
[[ "${plain_output}" == "sfs 0.8.70" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.70" "version output"
assert_contains_text "${output}" "latest 0.8.70" "latest output"
...
assert_contains_text "${output}" "cite-then-validate" "0.8.70 headline opening clause"
...
assert_contains_text "${fallback_output}" "인용 검증" "0.8.70 release-notes distinctive clause"
