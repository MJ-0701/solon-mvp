...
[[ "${plain_output}" == "sfs 0.12.0" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.12.0" "version output"
assert_contains_text "${output}" "latest 0.12.0" "latest output"
...
assert_contains_text "${output}" "A five-source blog insight batch hardens the operating loop" "0.12.0 headline opening clause"
...
assert_contains_text "${fallback_output}" "루프 규율·리스크 프리플라이트·모델 교체 감사" "0.12.0 release-notes distinctive clause"
