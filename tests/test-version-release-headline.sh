...
[[ "${plain_output}" == "sfs 0.8.60" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.60" "version output"
assert_contains_text "${output}" "latest 0.8.60" "latest output"
...
assert_contains_text "${output}" "Cost Signals" "0.8.60 headline opening clause"
...
assert_contains_text "${fallback_output}" "세션 비용 신호" "0.8.60 release-notes distinctive clause"
