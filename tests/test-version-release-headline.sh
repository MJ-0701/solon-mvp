...
[[ "${plain_output}" == "sfs 0.13.1" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.13.1" "version output"
assert_contains_text "${output}" "latest 0.13.1" "latest output"
...
assert_contains_text "${output}" "The thin indexes catch up with what 0.13.0 shipped" "0.13.1 headline opening clause"
...
assert_contains_text "${fallback_output}" "영문 index 의" "0.13.1 release-notes distinctive clause (first line only — fallback headline)"
