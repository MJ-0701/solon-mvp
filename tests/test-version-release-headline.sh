...
[[ "${plain_output}" == "sfs 0.8.66" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.66" "version output"
assert_contains_text "${output}" "latest 0.8.66" "latest output"
...
assert_contains_text "${output}" "map-vs-territory unknowns loop" "0.8.66 headline opening clause"
...
assert_contains_text "${fallback_output}" "unknowns 4분면 루프" "0.8.66 release-notes distinctive clause"
