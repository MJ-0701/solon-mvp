...
[[ "${plain_output}" == "sfs 0.10.1" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.10.1" "version output"
assert_contains_text "${output}" "latest 0.10.1" "latest output"
...
assert_contains_text "${output}" "Two rounds of independent Codex review" "0.10.1 headline opening clause"
...
assert_contains_text "${fallback_output}" "Codex 독립 리뷰 2라운드" "0.10.1 release-notes distinctive clause"
