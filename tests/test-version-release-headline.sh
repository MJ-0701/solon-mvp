...
[[ "${plain_output}" == "sfs 0.15.0" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.15.0" "version output"
assert_contains_text "${output}" "latest 0.15.0" "latest output"
...
assert_contains_text "${output}" "A four-source blog batch works the judgment layer" "0.15.0 headline opening clause"
...
assert_contains_text "${fallback_output}" "에이전트가 **언제 말을 걸고" "0.15.0 release-notes distinctive clause (first line only — fallback headline)"
