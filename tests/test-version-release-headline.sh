...
[[ "${plain_output}" == "sfs 0.11.1" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.11.1" "version output"
assert_contains_text "${output}" "latest 0.11.1" "latest output"
...
assert_contains_text "${output}" "The unknowns-loop contracts shipped in 0.11.0 gain runtime signals" "0.11.1 headline opening clause"
...
assert_contains_text "${fallback_output}" "정책 계약이 런타임 신호와 실행 발판을 얻습니다" "0.11.1 release-notes distinctive clause"
