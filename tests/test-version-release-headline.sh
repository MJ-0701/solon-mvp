...
[[ "${plain_output}" == "sfs 0.8.57" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.57" "version output"
assert_contains_text "${output}" "latest 0.8.57" "latest output"
...
assert_contains_text "${output}" "product-shape wiki coverage" "0.8.57 headline opening clause"
...
assert_contains_text "${fallback_output}" "위키 보강" "0.8.57 release-notes distinctive clause"
