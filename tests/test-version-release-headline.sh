...
[[ "${plain_output}" == "sfs 0.8.14" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.14" "version output"
assert_contains_text "${output}" "latest 0.8.14" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline 0.8.13 자율주행 루프 배치가 Stop hook template" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 0.8.13 자율주행 루프 배치가 Stop hook template" "release notes fallback headline"
