...
[[ "${plain_output}" == "sfs 0.14.2" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.14.2" "version output"
assert_contains_text "${output}" "latest 0.14.2" "latest output"
...
assert_contains_text "${output}" "A brew install could not read its own changelog" "0.14.2 headline opening clause"
...
assert_contains_text "${fallback_output}" "brew 로 설치한 sfs 가" "0.14.2 release-notes distinctive clause (first line only — fallback headline)"
