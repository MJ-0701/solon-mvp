...
[[ "${plain_output}" == "sfs 0.8.65" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.65" "version output"
assert_contains_text "${output}" "latest 0.8.65" "latest output"
...
assert_contains_text "${output}" "bash-bridge prelude" "0.8.65 headline opening clause"
...
assert_contains_text "${fallback_output}" "bash 브리지" "0.8.65 release-notes distinctive clause"
