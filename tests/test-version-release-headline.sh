...
[[ "${plain_output}" == "sfs 0.8.68" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.68" "version output"
assert_contains_text "${output}" "latest 0.8.68" "latest output"
...
assert_contains_text "${output}" "knob diagnostic ladder" "0.8.68 headline opening clause"
...
assert_contains_text "${fallback_output}" "knob 에스컬레이션" "0.8.68 release-notes distinctive clause"
