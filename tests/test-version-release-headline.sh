...
[[ "${plain_output}" == "sfs 0.12.1" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.12.1" "version output"
assert_contains_text "${output}" "latest 0.12.1" "latest output"
...
assert_contains_text "${output}" "Docs catch up with the last three releases" "0.12.1 headline opening clause"
...
assert_contains_text "${fallback_output}" "Unknowns 루프 사용자 가이드" "0.12.1 release-notes distinctive clause"
