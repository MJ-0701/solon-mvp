...
[[ "${plain_output}" == "sfs 0.8.69" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.69" "version output"
assert_contains_text "${output}" "latest 0.8.69" "latest output"
...
assert_contains_text "${output}" "declares the invariants" "0.8.69 headline opening clause"
...
assert_contains_text "${fallback_output}" "invariant 선언" "0.8.69 release-notes distinctive clause"
