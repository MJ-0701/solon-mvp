...
[[ "${plain_output}" == "sfs 0.7.10" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.7.10" "version output"
assert_contains_text "${output}" "latest 0.7.10" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline Session-transfer durable-handoff enumeration" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 이번 버전은 세션 인계 누락" "release notes fallback headline"
