...
[[ "${plain_output}" == "sfs 0.13.0" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.13.0" "version output"
assert_contains_text "${output}" "latest 0.13.0" "latest output"
...
assert_contains_text "${output}" "A seven-source blog insight batch hardens the operating loop" "0.13.0 headline opening clause"
...
assert_contains_text "${fallback_output}" "블로그 인사이트 7건을 한 배치로" "0.13.0 release-notes distinctive clause"
