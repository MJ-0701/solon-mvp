...
[[ "${plain_output}" == "sfs 0.15.4" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.15.4" "version output"
assert_contains_text "${output}" "latest 0.15.4" "latest output"
...
assert_contains_text "${output}" "Upgrade now preserves every active-sprint flow and evidence event" "0.15.4 headline opening clause"
...
assert_contains_text "${fallback_output}" "활성 sprint 원장의 flow·evidence 기록이 upgrade 중 사라지지 않도록 고친" "0.15.4 release-notes distinctive clause (first line only — fallback headline)"
