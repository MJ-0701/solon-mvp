...
[[ "${plain_output}" == "sfs 0.8.55" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.55" "version output"
assert_contains_text "${output}" "latest 0.8.55" "latest output"
...
assert_contains_text "${output}" "aligned to the real-Windows-verified exec form (issue #9)" "0.8.55 headline opening clause"
...
assert_contains_text "${fallback_output}" "byte 단위로 일치" "0.8.55 release-notes distinctive clause"
