...
[[ "${plain_output}" == "sfs 0.8.56" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.8.56" "version output"
assert_contains_text "${output}" "latest 0.8.56" "latest output"
...
assert_contains_text "${output}" "agent-identity access model" "0.8.56 headline opening clause"
...
assert_contains_text "${fallback_output}" "구획 단위 권한" "0.8.56 release-notes distinctive clause"
