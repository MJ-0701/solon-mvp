...
[[ "${plain_output}" == "sfs 0.6.111" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.6.111" "version output"
assert_contains_text "${output}" "latest 0.6.111" "latest output"
assert_contains_text "${output}" "status up-to-date" "status output"
assert_contains_text "${output}" "installed_release_headline Legit small review findings now stay on autopilot" "installed release headline"
...
assert_contains_text "${fallback_output}" "installed_release_headline 이번 버전은 작은 review finding" "release notes fallback headline"
