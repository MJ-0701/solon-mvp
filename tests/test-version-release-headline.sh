...
[[ "${plain_output}" == "sfs 0.16.2" ]] || fail "plain version output changed: ${plain_output}"
assert_contains_text "${output}" "sfs 0.16.2" "version output"
assert_contains_text "${output}" "latest 0.16.2" "latest output"
...
assert_contains_text "${output}" "ADR-aware daily handoffs now refresh for each completed work unit" "0.16.2 headline opening clause"
...
assert_contains_text "${fallback_output}" 'ADR-aware daily handoffs / ADR 연동 일일 인계가 완료된 각 작업 단위마다 자동으로 갱신되고 retro에서 최종화되는 patch release입니다.' "0.16.2 release-notes distinctive clause (first line only — fallback headline)"
