...
out="$(PATH="${tmp}/bin:${PATH}" FAKE_GH_MODE=present bash "${SCRIPT}" --version 0.6.127 --mode push)"
...
out="$(PATH="${tmp}/bin:${PATH}" FAKE_GH_MODE=absent bash "${SCRIPT}" --version 0.6.127 --mode push)"
...
printf '%s\n' "${out}" | grep -q 'verify-product-release.sh --version 0.6.127' || fail "manual_required should route to verifier"

if PATH="${tmp}/bin:${PATH}" FAKE_GH_MODE=absent bash "${SCRIPT}" --version 0.6.127 --require-workflow >/dev/null 2>&1; then
  fail "--require-workflow should fail when secret is absent"
fi

out="$(PATH="${tmp}/bin:${PATH}" FAKE_GH_MODE=fail bash "${SCRIPT}" --version 0.6.127 --mode pr)"
