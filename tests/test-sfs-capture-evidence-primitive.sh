#!/usr/bin/env bash
# tests/test-sfs-capture-evidence-primitive.sh — capture is evidence primitive, not lifecycle flow.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DIST_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
SFS_BIN="${DIST_DIR}/bin/sfs"
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sfs-capture-evidence.XXXXXX")"
...
assert_contains "${log_path}" "## Evidence Capture" "capture section"
assert_contains "${log_path}" "Capture 는 lifecycle 단계가 아니다" "not lifecycle section"
...
[[ "${evidence_count}" == "4" ]] || fail "evidence_capture events should not collapse; got ${evidence_count}"
...
evidence_count="$(grep -c '"type":"evidence_capture"' .sfs-local/events.jsonl || true)"
[[ "${evidence_count}" == "5" ]] || fail "explicit sprint capture should append fifth event; got ${evidence_count}"
assert_contains "${log_path}" "Explicit sprint capture still records to the named sprint." "explicit sprint capture text"

echo "test-sfs-capture-evidence-primitive: OK"
