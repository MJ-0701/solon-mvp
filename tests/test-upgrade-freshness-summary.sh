...
assert_contains "${upgrade_context}" "Version summaries must be release-anchored" "upgrade context release anchored"
assert_contains "${upgrade_context}" "installed_release_headline" "upgrade context installed headline"
assert_contains "${upgrade_context}" "Never infer the" "upgrade context no memory inference"
assert_contains "${upgrade_context}" "Do not say \"no major invariant changed\"" "upgrade context no unchecked invariant claim"
...
assert_contains "${upgrade_context}" "AC/ADR subset ownership" "upgrade context AC/ADR subset"
assert_contains "${upgrade_context}" "expected tests/evidence" "upgrade context expected evidence"
assert_contains "${upgrade_context}" "output report path" "upgrade context report path"
assert_contains "${upgrade_context}" "merge/conflict policy" "upgrade context merge policy"
...
for file in "${install_script}" "${upgrade_script}"; do
  ...
  assert_contains "${file}" "AC/ADR subset ownership" "completion AC/ADR subset ${file}"
  assert_contains "${file}" "expected tests/evidence" "completion expected evidence ${file}"
  assert_contains "${file}" "output report path" "completion report path ${file}"
  assert_contains "${file}" "merge/conflict policy" "completion merge policy ${file}"
