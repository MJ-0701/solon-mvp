...
for file in "${docs[@]}"; do
  ...
  assert_contains "${file}" "gpt-5.3-codex" "docs codex coding helper ${file}"
  assert_contains "${file}" "gpt-5.3-codex-spark" "docs codex spark ${file}"
  assert_contains "${file}" "worker" "docs worker role ${file}"
  assert_contains "${file}" "helper" "docs helper role ${file}"
  assert_contains "${file}" "Sonnet 4.6" "docs claude sonnet 4.6 ${file}"
  assert_contains "${file}" "Haiku" "docs claude haiku helper ${file}"
  assert_contains "${file}" "gemini-3.1-pro-preview" "docs gemini pro route ${file}"
  assert_contains "${file}" "gemini-3-flash-preview" "docs gemini coding route ${file}"
  assert_contains "${file}" "gemini-3.1-flash-lite" "docs gemini economy route ${file}"
  assert_contains "${file}" "3.x" "docs no gemini below 3x fallback contract ${file}"
  assert_not_contains "${file}" "gemini-3.5-flash" "docs no unsupported gemini 3.5 route ${file}"
  ...
  assert_not_contains "${file}" "gemini-2.5" "docs no gemini 2.5 model name ${file}"
  ...
  assert_not_contains "${file}" "0.6.26 기준" "docs no stale 0.6.26 기준 ${file}"
  ...
  assert_not_contains "${file}" "As of 0.6.27" "docs no stale As of 0.6.27 ${file}"
...
assert_contains "${DIST_DIR}/GUIDE.md" "일반 worker 기본값이 아니라" "GUIDE coding helper boundary"
assert_contains "${DIST_DIR}/GUIDE.md" "판단이 필요 없는 기계적 구현 보조 작업" "GUIDE spark helper boundary"
...
assert_contains "${DIST_DIR}/docs/en/guide.md" "repo-aware coding helper" "EN guide coding helper boundary"
assert_contains "${DIST_DIR}/docs/en/guide.md" "judgment-free mechanical" "EN guide spark helper boundary"
...
assert_contains "${changelog}" "Sonnet 4.6" "CHANGELOG claude sonnet 4.6"
assert_contains "${changelog}" "Haiku" "CHANGELOG claude haiku helper"
assert_contains "${changelog}" "gemini-3.1-pro-preview" "CHANGELOG gemini pro route"
assert_contains "${changelog}" "gemini-3-flash-preview" "CHANGELOG gemini coding route"
assert_contains "${changelog}" "gemini-3.1-flash-lite" "CHANGELOG gemini economy route"
assert_contains "${changelog}" "3.x" "CHANGELOG no gemini below 3x fallback"
assert_contains "${changelog}" "self-agent top-model CPO" "CHANGELOG self-agent top-model CPO"
assert_contains "${release_notes}" "advisor 호출은 self-CPO PASS 를 대체하지 않습니다" "release notes self CPO"
assert_contains "${release_notes}" "Sonnet 4.6" "release notes claude sonnet 4.6"
assert_contains "${release_notes}" "Haiku" "release notes claude haiku helper"
assert_contains "${release_notes}" "gemini-3.1-pro-preview" "release notes gemini pro route"
assert_contains "${release_notes}" "gemini-3-flash-preview" "release notes gemini coding route"
assert_contains "${release_notes}" "gemini-3.1-flash-lite" "release notes gemini economy route"
assert_contains "${release_notes}" "3.x 미만 fallback 은 쓰지 않습니다" "release notes no gemini below 3x fallback"
assert_contains "${release_notes}" "self-agent top-model CPO" "release notes self-agent top-model CPO"
