...
> `report.md` 에 남고, 본 파일 원문은 `.sfs-local/archives/` 로 이동해 보존된다.
...
작업 중 문서는 노트패드처럼 쌓여도 되지만, 완료 후에는 보고서 중심으로 정리되어야 한다.

- [x] R1: 기존 SFS workbench 문서를 삭제 없이 정리하는 명령이 필요하다.
- [x] R2: 완료된 sprint 는 `report.md` 중심으로 읽히고, brainstorm/plan/implement/log/review 는 archive 로 이동해 표면에서 사라져야 한다.
...
- [x] AC1: `sfs tidy` 는 기본 실행 시 dry-run 으로 동작하고 파일을 수정하지 않는다 — verify by dry-run smoke.
- [x] AC2: `sfs tidy --apply` 는 대상 sprint 의 원문 workbench/tmp 산출물을 archive 위치로 이동해 보존한다 — verify by archived files exist and original content is recoverable.
- [x] AC3: `sfs tidy --apply` 후 sprint 폴더에는 남겨야 할 문서만 남는다 (`report.md`, `retro.md`, 기타 durable artifacts) — verify by brainstorm/plan/implement/log/review removed from visible sprint folder.
- [x] AC4: `sfs tidy` 는 `--sprint <id>` 와 현재 sprint 기본값을 지원하고, 완료/미완료 sprint 에 대한 안전 메시지를 낸다 — verify by smoke on active and named sprint.
- [x] AC5: README/GUIDE/skill/command docs 에 `sfs upgrade`, `sfs version --check`, SFS = Solo Founder System, release note 확인 경로가 명시된다 — verify by `rg`.
- [x] AC6: `scripts/cut-release.sh` 는 대상 버전의 CHANGELOG 또는 release note entry 가 없을 때 preflight 에서 실패한다 — verify by release preflight exit 8 with missing entry.
- [x] AC7: CHANGELOG 규칙은 최소 `Added`, `Changed`, `Fixed` 를 포함하고, 필요 시 `Deprecated`, `Removed`, `Security` 를 허용한다 — verify by documented template and release script pattern.
- [x] Anti-AC: 어떤 경로에서도 사용자 동의 없는 삭제, `rm` 기반 청소, durable report 생성 없이 workbench 정리를 하지 않는다.
...
- **In scope**:
  ...
  - archive-first 보존 정책과 "남겨야 할 문서만 남기는" visible folder cleanup.
  - README/GUIDE/SFS template/skill/agent command docs 갱신.
  - CHANGELOG release note policy 와 cut-release preflight.
  - sandbox smoke: dry-run 무변경, apply archive 보존/visible cleanup, release note missing fail.
- **Out of scope**:
...
- **CPO Evaluator 가 검증할 것**:
  ...
  - AC 검증 방법: dry-run diff 없음, apply archive 생성 + visible cleanup, docs `rg`, release preflight fail/pass, version docs 확인.
