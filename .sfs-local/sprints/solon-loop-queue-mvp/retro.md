...
last_touched_at: "2026-05-01T08:55:00+09:00"
closed_at: ""        # filled by /sfs retro --close (⚠️ F-4b 미해결 상태에서는 exit 8 risk)
---
...
- [ ] **events.jsonl 마지막 entry 가 본 sprint 의 `gate_review G5:pass` (또는 `sprint_close`) 이도록 보장** — ⚠️ 현재 마지막 entry = `2026-04-30T23:18:43+09:00 plan_open`. F-4b 미해결 상태에서는 `sfs retro --close` 가 events.jsonl 의 `review_open` 부재로 exit 8 risk. **선결**: §4 결정 대기 항목 (F-4b backfill) 의 사용자 결정.
- [ ] **`closed_at` frontmatter 기록** — `/sfs retro --close` 가 자동 채움. 본 retro draft 시점에는 빈 값 (frontmatter `closed_at: ""`).
- [ ] **HANDOFF / sessions log 에 본 sprint 결과 link 1줄 추가** — 사용자 manual.
- [ ] **F-3 / F-4a / F-4b 의 별도 sprint 분기 등재** — §4 분기되는 WU/sprint 의 가칭 4 항목을 backlog (queue/pending/) 로 enqueue 또는 sprint dir 신설. 사용자 결정 후 진행.

> **본 retro 의 status**: G5 draft 완료. close 진입은 §5 의 첫 항목 (F-4b backfill 결정) 이 사용자 손에서 끝난 후. 그 전까지 `closed_at` = "" 유지.
