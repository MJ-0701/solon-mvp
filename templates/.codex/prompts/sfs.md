...
7. AI-era fundamentals apply across all gates, not only implement: shared design
   ...
   gray-box delegation. DDD/TDD is a product-level engineering floor: product
   behavior, domain language, behavior boundary, and first evidence are named
   before worker handoff; DDD-lite code boundaries apply when code is touched.
...
10. Do not show a question/recommendation-only choice table. When multiple
    ...
    alternatives.
11. Never ask the user to confirm a compact option bundle such as `A/A/A/C/C`,
...
12. Taxonomy is a product function, not an org division or copy polish. Match
...
13. If a required command argument is missing, ask one plain-language question
