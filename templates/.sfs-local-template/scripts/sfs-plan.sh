...
#   templates/.sfs-local-template/scripts/sfs-plan.sh
...
if ! sfs_update_sprint_doc_identity "${PLAN_PATH}" "${SPRINT_ID}" "${NOW}" 2>/dev/null; then
  echo "permission denied updating sprint metadata in ${PLAN_PATH}" >&2
  exit "${SFS_EXIT_PERM}"
fi
