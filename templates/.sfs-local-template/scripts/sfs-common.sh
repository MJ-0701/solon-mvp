...
#
# Visibility: distribution template (OSS fork inclusion 결정은 후속).
...
# sfs_event_json_string_field FIELD LINE — extract a simple JSON string value
# from SFS-owned compact JSONL lines. SFS event payloads are single-line and
# generated by this runtime; this is not a general JSON parser.
sfs_event_json_string_field() {
...
sfs_event_same_compaction_key() {
  ...
  if [[ -n "${gate}" && "${existing}" != *"\"gate_id\":\"${gate}\""* ]]; then
    return 1
  fi
  if [[ -n "${division}" && "${existing}" != *"\"division\":\"${division}\""* ]]; then
    return 1
  fi
  if [[ -n "${decision}" && "${existing}" != *"\"decision_id\":\"${decision}\""* ]]; then
    return 1
  fi
...
# WARNING: AI 자율 호출 금지 — 사용자가 `/sfs retro` 명시 호출 시에만 동작.
