...
Commands: status, start, guide, auth, brainstorm, plan, implement, review, decision, report, tidy, retro, commit, loop
...
case "${cmd}" in
  help|-h|--help)
    ...
    exit 0
    ;;
  status|start|guide|auth|brainstorm|plan|implement|review|decision|report|tidy|retro|commit|loop)
