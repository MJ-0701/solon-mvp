...
Commands: status, start, guide, auth, profile, division, adopt, brainstorm, plan, implement, review, decision, capture, note, ingest, report, report-bug, flowcheck, healthcheck, recall, event, tidy, retro, commit, loop
...
case "${cmd}" in
  help|-h|--help)
    ...
    exit 0
    ;;
  status|start|guide|auth|profile|division|adopt|brainstorm|plan|implement|review|decision|ingest|report|report-bug|flowcheck|healthcheck|recall|event|tidy|retro|commit|loop)
