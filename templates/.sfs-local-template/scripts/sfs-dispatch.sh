...
Commands: status, start, guide, auth, profile, division, adopt, brainstorm, plan, implement, review, decision, capture, note, report, report-bug, flowcheck, event, tidy, retro, commit, loop
...
case "${cmd}" in
  help|-h|--help)
    ...
    exit 0
    ;;
  status|start|guide|auth|profile|division|adopt|brainstorm|plan|implement|review|decision|report|report-bug|flowcheck|event|tidy|retro|commit|loop)
