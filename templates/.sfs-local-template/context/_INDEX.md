...
- `policies/unknowns-and-deviations.md` — map-vs-territory unknowns quadrant + blind-spot pass at plan preflight, conservative deviation log during implementation (lessons SIGNAL input), post-implementation explainer/quiz comprehension gate (all signal-only).
...
- `policies/token-harness.md` — token/context hygiene, semantic search, repeated mistake guardrails; KNOB_DIAGNOSTIC_LADDER (failure escalation: context/skills → effort ↑ → model tier ↑, routine stretches downshift).
