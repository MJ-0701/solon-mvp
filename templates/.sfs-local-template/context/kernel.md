...
- Handoff-only scope is a stop contract: if the user asks only to create/update a handoff, next-session brief, session report, or `인계문서`, write the artifact, record current state/blockers/first next command, clean heartbeat/automation evidence when relevant, then stop. Do not start PR polling, review retriggers, merges, implementation, deploy, or monitor loops unless the same user request explicitly asks to continue that work.
