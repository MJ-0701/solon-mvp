...
summary: plan 단계에서 6개 council role이 실제 설계 검토를 수행하도록 하는 계약 팩.
language: ko
load_when:
  - plan
  - Gate 3
...
3. 각 council role마다 finding, risk flag, AC/files/evidence mapping,
   `asset_candidate`, waiver/N/A, next action 을 기록한다.
4. 해당 row 에 트리거가 있을 때만 deep pack 을 연다.
5. 관련 council role의 finding/evidence/waiver가 없으면 Gate 3는 partial이다.

## Council role별 산출

organization division은 다섯 개(strategy-pm, dev, QA, design, infra)다.
taxonomy는 cross-cutting product function/lens이며, 여섯 role은 모두 required council role이다.
...
명시적 gap/waiver 중 하나를 적는다. 이것이 six-role council의 존재 이유다.
...
- council role별 asset candidate 또는 concrete N/A reason
...
six-role council 표를 장식으로 채우지 않는다. 관련 없으면 concrete reason과 함께
