---
name: sfs
description: Solon SFS command router for Codex. Dispatch `$sfs` / `sfs` / visible `/sfs` text to the deterministic SFS adapter (`sfs.cmd` on Windows PowerShell/cmd, `sfs` elsewhere), then resolve routed context with `sfs context path ...` or `sfs.cmd context path ...`. `profile` is a narrow SFS.md project-overview refinement.
---
