...
> **0.6.49 기준** brew/scoop 한 줄이면 Claude Code, Gemini CLI, Codex CLI 가 모두
...
0.6.49 기준으로는 분야별 지식팩이 실제 안내로 채워져 있습니다. 사용자가 backend, QA, infra, 재무,
...
[Windows SFS 래퍼 장애 요약 보고서](./docs/ko/windows-wrapper-incident-0.6.49.md) 에 정리되어
있습니다.
...
0.6.49 기준으로는 Windows PowerShell/cmd 의 성공 기준을 `sfs.cmd` 경로로 고정합니다.
...
소유하므로 batch label forwarding, `-File ... %*`, `-Command @args`, empty `%1..%n`, generated bare `sfs` shim, generated `sfs.cmd` shim, generated shim -> packaged `.cmd` 때문에
...
0.6.49 기준 brew/scoop 가 세 CLI 모두에 자동 등록합니다. 그래도
