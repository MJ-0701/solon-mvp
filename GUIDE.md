...
> **0.6.43 기준** brew/scoop 한 줄이면 Claude Code, Gemini CLI, Codex CLI 가 모두
...
0.6.43 기준으로는 분야별 지식팩이 실제 안내로 채워져 있습니다. 사용자가 backend, QA, infra, 재무,
...
[Windows SFS 래퍼 장애 요약 보고서](./docs/ko/windows-wrapper-incident-0.6.43.md) 에 정리되어
있습니다.
...
0.6.43 기준으로는 `sfs.cmd` 가 label 없는 thin PowerShell trampoline 으로만 동작하고,
PowerShell `-Command "& $env:SFS_NATIVE_SCRIPT @args"` 로 `sfs.ps1` 에 진입합니다. `sfs.ps1` 이
positional array param, `$args`, `$MyInvocation.UnboundArguments` 순서로 인자를 정규화하고,
`version`, `status`, `guide`, `context`, Scoop self-upgrade, Bash fallback 을 소유하므로
batch label forwarding 과 `-File ... %*` 때문에 `sfs.cmd context cat ...` 과
`sfs.cmd start ...` 가 usage 만 출력하던 Windows wrapper 인자 손실을 막습니다.
이미 0.6.41 이하의 깨진 wrapper 에 갇혀 `sfs.cmd update` 자체가 usage 만 출력한다면, 최초 1회는
PowerShell 에서 `scoop update sfs` 를 직접 실행한 뒤 프로젝트 폴더에서
`sfs.cmd upgrade --no-self-upgrade` 를 실행하세요.

0.6.43 기준 brew/scoop 가 세 CLI 모두에 자동 등록합니다. 그래도
