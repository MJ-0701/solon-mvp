...
> **0.6.42 기준** brew/scoop 한 줄이면 Claude Code, Gemini CLI, Codex CLI 가 모두
...
0.6.42 기준으로는 분야별 지식팩이 실제 안내로 채워져 있습니다. 사용자가 backend, QA, infra, 재무,
...
[Windows SFS 래퍼 장애 요약 보고서](./docs/ko/windows-wrapper-incident-0.6.42.md) 에 정리되어
있습니다.
...
0.6.42 기준으로는 `sfs.cmd` 가 label 없는 thin PowerShell trampoline 으로만 동작하고,
`sfs.ps1` 이 Windows PowerShell `-File` 호출 인자를 `$args` /
`$MyInvocation.UnboundArguments` 로 직접 읽습니다. `sfs.ps1` 이 `version`, `status`, `guide`,
`context`, Scoop self-upgrade, Bash fallback 을 소유하므로 batch label forwarding 때문에
...
0.6.42 기준 brew/scoop 가 세 CLI 모두에 자동 등록합니다. 그래도
