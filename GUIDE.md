...
> **0.6.39 기준** brew/scoop 한 줄이면 Claude Code, Gemini CLI, Codex CLI 가 모두
...
0.6.39 기준으로는 분야별 지식팩이 실제 안내로 채워져 있습니다. 사용자가 backend, QA, infra, 재무,
...
[Windows SFS 래퍼 장애 요약 보고서](./docs/ko/windows-wrapper-incident-0.6.39.md) 에 정리되어
있습니다.
...
0.6.39 부터는 `sfs.ps1` 이 Windows PowerShell `-File` 호출 인자를 `$args` /
...
0.6.39 기준 brew/scoop 가 세 CLI 모두에 자동 등록합니다. 그래도
