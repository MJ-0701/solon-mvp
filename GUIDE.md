...
## 1. 첫 5분 — `SFS.md` 프로젝트 개요 확인

`sfs init` 과 `sfs upgrade` 는 `package.json`, `next.config.*`, `prisma/schema.prisma`,
`pyproject.toml`, `Dockerfile`, `vercel.json`, `README.md`, `docs/` 같은 고정 위치를 먼저
보고 프로젝트 개요를 채운다. 개발 프로젝트가 아니어도 문서/지식 작업공간이면 그에 맞게 적는다.

그래도 남는 placeholder 는 사용자가 직접 채우거나, AI runtime 에서 이 좁은 명령만 실행한다:

```bash
sfs profile
# shell-only quick apply 가 필요하면:
sfs profile --apply
```

`SFS.md` 맨 위 프로젝트 개요는 이런 형태다:

```markdown
- **이름**: `<PROJECT-NAME>`
- **유형**: `<PROJECT-TYPE>` (웹 제품 / 문서 운영 / 내부 자동화 / 연구 프로젝트)
- **단계**: `<PROJECT-STAGE>` (기존 프로젝트에 Solon 도입 / MVP / 운영 중 / 실험)
- **환경**: `<PROJECT-ENVIRONMENT>` (기술 stack, 문서 도구, 협업 환경 등)
- **핵심 산출물**: `<PROJECT-OUTPUT>` (웹 앱 / API / 문서 / 데이터셋 / playbook)
- **공유/운영 방식**: `<PROJECT-DELIVERY>` (Vercel / Docker / GitHub Pages / 로컬 문서)
```
...
- **유형**: `소프트웨어/웹 제품`
- **단계**: `Solon 운영 도입 초기`
- **환경**: `Node.js + TypeScript + Next.js + Tailwind CSS + Prisma + Postgres`
- **핵심 산출물**: `웹 앱`
- **공유/운영 방식**: `Vercel`
```
...
- **유형**: `문서/지식 작업공간`
- **단계**: `Solon 운영 도입 초기`
- **환경**: `Markdown/Git 작업공간`
- **핵심 산출물**: `문서/운영 기록`
- **공유/운영 방식**: `Git 또는 문서 공유`
```
...
Homebrew 로 global `sfs` CLI 는 설치됐지만, 현재 프로젝트에는 아직 Solon 파일을
주입하지 않은 상태입니다. 프로젝트 루트에서:
...
`brew install` 은 Mac 전체에 CLI 를 설치하고, `sfs init --yes` 는 현재 repo 에
`SFS.md`, `.sfs-local/`, agent adapter 를 생성합니다. `sfs upgrade` 는 이미 init 된
...
`sfs init --yes` / `sfs upgrade --yes` 는 이 질문까지 묻지 않고 `current_model` fallback 을
유지합니다. 대화형으로 설정할 때는 Claude 권장값뿐 아니라 Codex 권장값과 Gemini 권장값을
같은 선택지에서 고를 수 있습니다.
...
아니. 프로젝트 루트에서 `sfs upgrade` 한 번만 실행한다. Homebrew 설치본이면 먼저
