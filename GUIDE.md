...
> **0.6.67 기준** brew/scoop 한 줄이면 Claude Code, Gemini CLI, Codex CLI 가 모두
...
0.6.67 기준으로는 분야별 지식팩이 실제 안내로 채워져 있습니다. 사용자가 backend, QA, infra, 재무,
세무, 회계 같은 말을 정확히 몰라도 괜찮습니다. Solon 을 쓰는 AI 가 작업 성격을 보고 필요한
관점만 읽고, 사용자에게는 평범한 질문과 판단 기준으로 풀어 설명하는 쪽이 기본입니다.

또 0.6.67 기준으로 source-driven 구현, stop-the-line 디버깅, deprecation/migration, shipping
...
0.6.67 기준으로는 Windows PowerShell/cmd 의 성공 기준을 `sfs.cmd` 경로로 고정합니다.
...
0.6.67 기준 brew/scoop 가 세 CLI 모두에 자동 등록합니다. 그래도
