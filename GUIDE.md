...
> **0.6.46 기준** brew/scoop 한 줄이면 Claude Code, Gemini CLI, Codex CLI 가 모두
...
0.6.46 기준으로는 분야별 지식팩이 실제 안내로 채워져 있습니다. 사용자가 backend, QA, infra, 재무,
...
[Windows SFS 래퍼 장애 요약 보고서](./docs/ko/windows-wrapper-incident-0.6.46.md) 에 정리되어
있습니다.
...
0.6.46 기준으로는 Scoop manifest 가 generated shim 을 packaged `bin\sfs.ps1` 로 직접 연결합니다.
`sfs.cmd` 는 직접 실행/호환용 thin PowerShell trampoline 으로 남고,
...
`sfs.ps1` 이 Scoop PowerShell shim positional args, env bridge, `$args`, `CMDCMDLINE`, `$MyInvocation.UnboundArguments` 순서로
인자를 정규화하고, `version`, `status`, `guide`, `context`, Scoop self-upgrade, Bash fallback 을
소유하므로 batch label forwarding, `-File ... %*`, `-Command @args`, empty `%1..%n`, generated shim -> packaged `.cmd` 때문에
...
0.6.46 기준 brew/scoop 가 세 CLI 모두에 자동 등록합니다. 그래도
