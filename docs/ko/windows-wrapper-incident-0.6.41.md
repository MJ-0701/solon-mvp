# Windows SFS 래퍼 장애 요약 보고서 (0.6.35 -> 0.6.41)

**Language**: 한국어 / [English](../en/windows-wrapper-incident-0.6.41.md)
...
종료해야 합니다. Windows PowerShell/cmd runtime script 는 BOM 없는 UTF-8 파서 오인을 피하도록
ASCII-safe 여야 합니다.
...
- `sfs.cmd` 는 인자를 별도 환경 변수에 저장해 재사용하지 않고, call-label `%*` 를 직접
  `sfs.ps1` 로 넘겨야 합니다.
...
- `tests/test-windows-wrapper-incident-report.sh` 는 이 보고서의 P1-P8 문제 요약, 0.6.41 문서 링크,
...
- `tests/test-docs-model-routing.sh` 는 source layout 과 Homebrew installed layout 의 문서 위치를
  함께 검증합니다.
- 0.6.41 은 Windows `sfs.cmd upgrade` self-replacement 수정, 설치본 incident-report 테스트
  layout 보강, batch same-line exit, ASCII-only Windows script, direct `%*` forwarding 보강까지
  포함한 최종 후속 기준선입니다.
