# Windows SFS 래퍼 장애 요약 보고서 (0.6.35 -> 0.6.42)

**Language**: 한국어 / [English](../en/windows-wrapper-incident-0.6.42.md)
...
- `sfs.cmd` 는 인자를 별도 환경 변수에 저장해 재사용하지 않아야 합니다. 0.6.42 기준으로는
...
- `tests/test-windows-wrapper-incident-report.sh` 는 이 보고서의 P1-P9 문제 요약, 0.6.42 문서 링크,
...
- `tests/test-docs-model-routing.sh` 는 source layout 과 Homebrew installed layout 의 문서 위치를
  함께 검증합니다.
- 0.6.42 는 Windows `sfs.cmd upgrade` self-replacement 수정, 설치본 incident-report 테스트
  layout 보강, batch same-line exit, ASCII-only Windows script, direct `%*` forwarding, 그리고
  batch label 제거까지 포함한 최종 후속 기준선입니다.
