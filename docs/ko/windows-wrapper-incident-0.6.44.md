# Windows SFS 래퍼 장애 요약 보고서 (0.6.35 -> 0.6.44)

**Language**: 한국어 / [English](../en/windows-wrapper-incident-0.6.44.md)
...
- 0.6.43 에서는 0.6.42 GitHub Windows Scoop smoke 의 추가 실패를 근거로 `sfs.cmd` 의 PowerShell
  ...
  0.6.43 Windows smoke 에서 다시 usage-only 로 실패했으므로 0.6.44 에서 supersede 했습니다.
...
- `tests/test-windows-agent-adapter-fallback.sh` 는 call-label dispatch 없는 `sfs.cmd -> numbered env bridge ->
  sfs.ps1 -> Bash runtime` bridge 와 raw Git Bash `%*` 비사용을 고정합니다. 또한 `sfs.cmd` 가 `call :...` 또는
...
- `tests/test-windows-wrapper-incident-report.sh` 는 이 보고서의 P1-P11 문제 요약, 0.6.44 문서 링크,
...
- `tests/test-docs-model-routing.sh` 는 source layout 과 Homebrew installed layout 의 문서 위치를
  함께 검증합니다.
- 0.6.44 는 Windows `sfs.cmd upgrade` self-replacement 수정, 설치본 incident-report 테스트
  layout 보강, batch same-line exit, ASCII-only Windows script, `SFS_ORIGINAL_ARGS` 제거,
  call-label dispatch 제거, `-File ... %*` / `-Command @args` 실패 학습, numbered env arg bridge 까지
  포함한 최종 후속 기준선입니다.
