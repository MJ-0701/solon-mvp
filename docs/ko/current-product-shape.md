...
Windows PowerShell/cmd 의 진입점은 `sfs.cmd` 입니다. 0.6.41 기준 Windows wrapper 는
...
경로로 넘깁니다. 0.6.41 기준 `sfs.ps1` 은 Windows PowerShell `-File` 인자를 `$args` /
`$MyInvocation.UnboundArguments` 로 직접 읽고, `sfs.cmd` 는 `SFS_ORIGINAL_ARGS` 캐시 없이
call-label `%*` 를 직접 전달합니다. 또한 `sfs.cmd` 가 PowerShell 호출 뒤 같은 parsed line 에서
종료하고 Windows runtime `.ps1` / `.cmd` 파일을 ASCII-safe 로 유지해 `context cat` / `start`
usage-only 회귀, batch tail fragment, PowerShell 5.1 parser 회귀를 함께 막습니다.
...
자세한 원인과 확인 절차는
[Windows SFS 래퍼 장애 요약 보고서](./windows-wrapper-incident-0.6.41.md) 에 정리되어 있습니다.
...
0.6.41 기준 backend, 전략/PM, QA, 디자인/frontend, infra/DevOps, 경영관리, taxonomy 지식팩은
