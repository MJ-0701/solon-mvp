...
macOS/Linux 처럼 `sfs` 를 씁니다. 0.6.53 기준 Scoop manifest 는 generated shim 이
...
`sfs.ps1` 이 `SFS_NATIVE_RAW_ARGS`, delayed-expansion `SFS_NATIVE_CMDLINE`, `CMDCMDLINE` 순서로
fallback 을 읽습니다. saved command-line parser 는 `&& sfs.cmd --help` 같은 `cmd.exe`
...
PowerShell 자동 `$args`, `CMDCMDLINE`, `$MyInvocation.UnboundArguments` 순서로 인자를 정규화하고, `version`, `status`, `guide`,
...
자세한 원인과 확인 절차는
[Windows SFS 래퍼 장애 요약 보고서](./windows-wrapper-incident-0.6.53.md) 에 정리되어 있습니다.
...
0.6.53 기준 backend, 전략/PM, QA, 디자인/frontend, infra/DevOps, 경영관리, taxonomy 지식팩은
