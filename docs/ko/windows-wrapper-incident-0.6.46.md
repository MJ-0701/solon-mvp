# Windows SFS 래퍼 장애 요약 보고서 (0.6.35 -> 0.6.46)

**Language**: 한국어 / [English](../en/windows-wrapper-incident-0.6.46.md)
...
| P13 | generated Scoop shim 이 packaged `bin\sfs.cmd` 를 target 으로 삼으면 원래 인자 꼬리를 끝까지 잃을 수 있음 | 0.6.45 GitHub Windows Scoop smoke run `25533332634` 에서 `CMDCMDLINE` fallback 이후에도 최초 `sfs version` 이 usage-only 로 실패 | Scoop manifest 의 primary shim target 을 `bin\sfs.ps1` 로 고정하고, `sfs.ps1` 은 `ValueFromRemainingArguments` positional args 를 받음. packaged `sfs.cmd` 는 직접 실행/호환용 trampoline 으로 유지 |
...
- `sfs.ps1` 은 단일 script param 에만 의존하지 않습니다. Scoop PowerShell shim 의 positional
  args, env bridge, automatic `$args`, `$MyInvocation.UnboundArguments`, nested array, accidental
...
- 0.6.46 에서는 0.6.45 Windows smoke 의 추가 실패를 근거로 Scoop manifest 의 primary `bin`
  ...
  `sfs.ps1` 을 직접 호출하고, `sfs.ps1` 은 `ValueFromRemainingArguments` positional args 를 받습니다.
  packaged `sfs.cmd` 는 직접 실행/호환용 trampoline 으로 남깁니다.
...
- `tests/test-windows-wrapper-incident-report.sh` 는 이 보고서의 P1-P13 문제 요약, 0.6.46 문서 링크,
...
- `tests/test-docs-model-routing.sh` 는 source layout 과 Homebrew installed layout 의 문서 위치를
  함께 검증합니다.
- 0.6.46 은 Windows `sfs.cmd upgrade` self-replacement 수정, 설치본 incident-report 테스트
  ...
  numbered env arg bridge, `CMDCMDLINE` fallback, Scoop primary `bin\sfs.ps1` shim target 까지 포함한
  최종 후속 기준선입니다.
