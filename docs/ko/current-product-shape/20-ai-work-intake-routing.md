...
문서가 약하거나 없던 프로젝트에서는 intake 가 memory formation 으로 시작합니다. 코드, git history,
test, config, deployment trace, 기존 메모를 먼저 확인하고 known/unknown/already-answered question 을
기록한 뒤에만 사용자에게 넓은 배경 설명을 다시 요청합니다.
