# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.45)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.45.md) / English
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P12 issue
  summary, the 0.6.45 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.45 is the final follow-up baseline that includes the Windows
  ...
  bridge, and the `CMDCMDLINE` fallback.
