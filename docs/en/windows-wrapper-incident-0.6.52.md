# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.52)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.52.md) / English
...
lets `sfs.ps1` read it as a raw-arg fallback.
Windows
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P19 issue
  summary, the 0.6.52 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.52 is the final follow-up baseline that includes the Windows
