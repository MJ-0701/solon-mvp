# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.47)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.47.md) / English
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P14 issue
  summary, the 0.6.47 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.47 is the final follow-up baseline that includes the Windows
  ...
  target, script-param removal, and the automatic `$args` primary path.
