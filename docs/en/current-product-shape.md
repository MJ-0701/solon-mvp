...
The Windows PowerShell/cmd entrypoint is `sfs.cmd`. As of 0.6.45, the wrapper
keeps `sfs.cmd` as a call-label-dispatch-free thin PowerShell trampoline and passes arguments
to `sfs.ps1` through the `SFS_NATIVE_ARGC` / `SFS_NATIVE_ARG_N` numbered env
bridge. If that is also empty, `sfs.ps1` reads the original `CMDCMDLINE` as the
last fallback. `sfs.ps1` owns both read-only commands and mutating commands such as
...
`-File ... %*` bridge, `-Command @args` bridge, and empty `%1..%n` path are no longer defaults because
...
file that Scoop replaces. `sfs.ps1` normalizes env bridge args, positional array
param args, `$args`, `CMDCMDLINE`, and `$MyInvocation.UnboundArguments`, and owns `version`,
...
[Windows SFS wrapper incident report](./windows-wrapper-incident-0.6.45.md).
...
As of 0.6.45, the backend, strategy/PM, QA, design/frontend, infra/DevOps,
