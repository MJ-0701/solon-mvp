...
The Windows PowerShell/cmd entrypoint is `sfs.cmd`. As of 0.6.44, the wrapper
...
bridge. `sfs.ps1` owns both read-only commands and mutating commands such as
...
`-File ... %*` bridge, and `-Command @args` bridge are no longer defaults because
...
param args, `$args`, and `$MyInvocation.UnboundArguments`, and owns `version`,
...
[Windows SFS wrapper incident report](./windows-wrapper-incident-0.6.44.md).
...
As of 0.6.44, the backend, strategy/PM, QA, design/frontend, infra/DevOps,
