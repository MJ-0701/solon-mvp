...
> **As of 0.6.49**, one `brew install` / `scoop install` lets Claude Code
...
Old projects can be upgraded into the lighter 0.6.49 shape. Use
`sfs upgrade --layout vendored` only when Solon package files must stay inside
the project.

0.6.49 also includes the filled division knowledge packs. The user does not need to choose
