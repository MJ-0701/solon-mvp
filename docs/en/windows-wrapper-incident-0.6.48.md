# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.48)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.48.md) / English
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P15 issue
  summary, the 0.6.48 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.48 is the final follow-up baseline that includes the Windows
  ...
  target, script-param removal, the automatic `$args` primary path, and the
  Windows PowerShell/cmd `sfs.cmd` contract.
