# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.41)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.41.md) / English
...
exit on the same parsed line after calling `sfs.ps1`. Windows PowerShell/cmd
runtime scripts must also stay ASCII-safe so BOM-less UTF-8 is not mis-parsed by
...
- `sfs.cmd` should not cache arguments into an environment variable for later
  forwarding; the call-label `%*` is the reliable Windows/Scoop path.
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P8 issue
  summary, the 0.6.41 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.41 is the final follow-up baseline that includes the Windows
