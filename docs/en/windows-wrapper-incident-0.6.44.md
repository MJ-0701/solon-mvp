# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.44)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.44.md) / English
...
- 0.6.43 uses the 0.6.42 GitHub Windows Scoop usage-only failure as the signal
  ...
  failed again in the real 0.6.43 Windows smoke, so 0.6.44 supersedes it.
...
- `tests/test-windows-agent-adapter-fallback.sh` locks the call-label-dispatch-free
  `sfs.cmd -> numbered env bridge -> sfs.ps1 -> Bash runtime` bridge, rejects raw Git Bash `%*`
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P11 issue
  summary, the 0.6.44 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.44 is the final follow-up baseline that includes the Windows
  ...
  `-Command @args` failure learnings, and the numbered env arg bridge.
