# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.46)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.46.md) / English
...
- `sfs.ps1` no longer depends on one script-param source. It normalizes Scoop
  PowerShell shim positional args, env bridge args, automatic `$args`,
...
- 0.6.46 uses the additional 0.6.45 Windows smoke failure to change the Scoop
  ...
  generated Scoop PowerShell shim now calls `sfs.ps1` directly, and `sfs.ps1`
  accepts `ValueFromRemainingArguments` positional args. Packaged `sfs.cmd`
  remains a direct-run compatibility trampoline.
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P13 issue
  summary, the 0.6.46 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.46 is the final follow-up baseline that includes the Windows
  ...
  bridge, the `CMDCMDLINE` fallback, and the Scoop primary `bin\sfs.ps1` shim
  target.
