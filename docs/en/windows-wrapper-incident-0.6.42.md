# Windows SFS Wrapper Incident Report (0.6.35 -> 0.6.42)

**Language**: [한국어](../ko/windows-wrapper-incident-0.6.42.md) / English
...
- `sfs.cmd` should not cache arguments into an environment variable for later
  forwarding. As of 0.6.42, it also avoids call-label forwarding entirely and
...
- `tests/test-windows-wrapper-incident-report.sh` verifies the P1-P9 issue
  summary, the 0.6.42 report links, and the Homebrew installed-layout fallback.
- `tests/test-docs-model-routing.sh` validates both source docs and Homebrew
  installed docs layout.
- 0.6.42 is the final follow-up baseline that includes the Windows
  ...
  direct `%*` forwarding, and batch-label removal.
