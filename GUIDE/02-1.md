...
긴 자율 작업을 맡기기 전에는 `sfs harness doctor` 로 현재 프로젝트의 agent entry, routed context,
council participation, memory, test, release/check rail 이 갖춰졌는지 확인할 수 있습니다.
구조를 파일로 남기고 싶으면 `sfs harness map --write` 를 실행해
`.sfs-local/harness/harness-map.md` 를 만듭니다.
...
`events.jsonl` 은 현재 sprint active ledger 일 때만 남습니다. close/tidy/adopt 가 닫힌 sprint
이벤트를 prune 하기 전에는 raw JSONL excerpt 를
`.sfs-local/archives/events/sprints/<sprint-id>.jsonl` 에 먼저 보존합니다. 이 경로는 tar 를
풀지 않고도 tail/grep 할 수 있는 로컬 타임라인입니다. 보존이 끝난 뒤에만 root active ledger 의
stale/orphan 이벤트를 제거하거나 빈 ledger 를 삭제합니다. 반복 cleanup evidence 는 날짜별로
`.sfs-local/archives/adopt/surface-cleanup/<yyyyMMdd>/surface-cleanup.tar.gz` 하나에 묶여
바깥 파일 트리를 어지럽히지 않습니다.

현재 Solon 은 조직 본부, 지식팩, review lens 를 같은 층위로 섞지 않습니다.
조직 division은 `dev`, `strategy-pm`, `qa`, `design`, `infra` 다섯 개입니다.
`.sfs-local/divisions.yaml` 은 기존 프로젝트 호환을 위한 6개 activation slot을
유지하지만, `taxonomy` slot은 조직 division이 아니라 foundational cross-cutting
product function/lens입니다. 여섯 role은 모두 필수 council participation role입니다. 실제 guidance 는
