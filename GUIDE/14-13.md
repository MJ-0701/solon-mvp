...
AI 시대의 설계 원칙은 [현재 제품 구조와 운영 흐름](./docs/ko/current-product-shape.md) 와
...
Codex `gpt-5.5` xhigh, Gemini `gemini-3.1-pro-preview` 입니다. Gemini 는 strategy/research/review 를
`gemini-3.1-pro-preview`, agentic coding/bounded 구현 helper 를 `gemini-3-flash-preview`,
relay/probe/economy helper 를 `gemini-3.1-flash-lite` 로 라우팅하며 3.x 미만 fallback 은 쓰지 않습니다. Helper-grade 단순 relay/누락 인자
...
relay, 요약, 작은 read-only helper 전용입니다. 실질 research 는 가능하면 Gemini 3.1 Pro Preview
researcher 로 보냅니다.
