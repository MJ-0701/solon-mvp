...
비용은 **먼저 한 달 재고, 그 다음 제한**합니다. 실사용을 한 달 관측하기 전에는 합리적인 한도를
알 수 없으므로, 첫 대응은 상한을 거는 게 아니라 무엇에 얼마가 나가는지 보는 것입니다. 판단 단위는
토큰이 아니라 **결과 하나당 비용** 입니다 — "에이전트 없이 했다면 얼마였나(안 했을 일 포함)",
"이건 어려운 일인가 그냥 양이 많은 일인가". 자세한 기준은 routed context 의
`policies/token-harness.md` KNOB_DIAGNOSTIC_LADDER 가 SSoT 입니다.
