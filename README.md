...
4. `/sfs plan`, `/sfs decision`, `/sfs review`, `/sfs retro` 로 작업 evidence 를 남깁니다.
5. `/sfs status` 와 `.sfs-local/events.jsonl` 로 현재 상태를 확인합니다.
6. 필요하면 Codex/Gemini/Claude 가 같은 문서 계약을 읽고 다음 작업을 이어갑니다.
