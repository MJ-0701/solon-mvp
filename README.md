...
> **0.5.96-product 부터**: `brew install` / `scoop install` 한 번으로
> Claude Code (`/sfs`), Gemini CLI (`sfs`), Codex CLI (`$sfs`) 가 모두
> 자동으로 동작합니다. 프로젝트 트리에 plugin/extension/skill 파일을
> 만들지 않습니다 — 모든 CLI 검색 surface 는 user-home 의 plugin
> cellar 에 살고, 작업 산출물 (`.sfs-local/`, `SFS.md`,
> `CLAUDE.md`/`AGENTS.md`/`GEMINI.md`) 만 프로젝트에 남습니다.
...
세 줄 모두 ✅ 면 `/sfs <command>` 를 어느 프로젝트에서든 바로 사용할 수
있습니다. 어떤 줄이 ⚠️ 면 그 옆에 출력되는 한 줄 recovery 명령을 그대로
실행하세요.
