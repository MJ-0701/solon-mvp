...
7. `/sfs report` 로 최종 작업보고서 `report.md` 를 만들고, `/sfs retro --close` 에서 workbench 문서를 compact 합니다. `brainstorm/plan/implement/log/review` 는 작업 중 화이트보드이고, 완료 후에는 `report.md` + `retro.md` 가 읽는 입구입니다.
...
Gemini/custom 은 해당 runtime profile 이름으로 agent별 override 할 수 있습니다. 설정을
건너뛰거나 나중에 하기로 하면 Solon 은 현재 런타임에서 사용자가 선택한 모델을 그대로 씁니다.
...
| `/sfs report [--sprint <id>] [--compact]` | 최종 작업보고서 `report.md` 생성/갱신. `--compact` 는 사용자 동의 후 workbench 문서를 redirect/stub 로 압축 |
| `/sfs retro` | 현재 sprint 의 `retro.md` 작성 또는 갱신 |
| `/sfs retro --close` | AI runtime 에서는 retro + report refinement 후, review 실행 여부 확인 + workbench compact + sprint close + auto commit. 이후 branch push/main 흡수는 Git Flow lifecycle 로 처리 |
...
13 명령 모두 동일 bash adapter SSoT 입니다. `/sfs` 는 Claude/Gemini 쪽 command shape 이고,
...
- `/sfs retro --close` 는 사용자가 명시적으로 호출했을 때만 report 기반 workbench compact, sprint close, auto commit 을 수행합니다.
