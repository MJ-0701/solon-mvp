...
5. CTO Generator 가 구현하고, `/sfs review --gate G4 --executor codex|gemini|claude` 로 CPO Evaluator review 를 실행하고 verdict 를 남깁니다. 수동 handoff 만 필요하면 `--prompt-only` 를 사용합니다.
...
수동 전달만 필요할 때는 `--prompt-only` 로 prompt/log 만 생성한다.
...
| `/sfs review --gate <id> [--executor <tool>] [--prompt-only]` | CPO Evaluator review run. full prompt 는 tmp prompt file에 저장하고 `review.md`에는 compact log/result만 남김 (id ∈ G-1..G5) |
...
현재 distribution version 은 `0.5.23-product` 입니다. `-mvp` suffix (0.5.0-mvp 까지) 는 기존 설치본
