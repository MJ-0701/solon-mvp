...
설치/업그레이드는 `SFS.md` 프로젝트 개요도 먼저 감지합니다. 개발 프로젝트면
`package.json`, `next.config.*`, `prisma/schema.prisma`, `vercel.json` 등을 보고, 문서/운영
프로젝트면 `README.md`/`docs/` 중심으로 채웁니다. 남은 placeholder 만 좁게 다시 맡기고
싶으면 AI runtime 에서 `sfs profile` 을 실행합니다.
...
`sfs init --yes` 처럼 순수 터미널에서 non-interactive 로 설치하면 이 질문은 생략되고
`current_model` fallback 으로 안전하게 시작합니다. 대화형 설정에서는 Claude 권장값을 가장 흔한
기본값으로 제공하지만, Codex 권장값과 Gemini 권장값도 같은 단계에서 직접 선택할 수 있습니다.
...
| `/sfs profile [--prompt-only|--apply]` | SFS.md 프로젝트 개요만 좁게 감지/보정. AI runtime 은 허용 파일만 읽고 `## 프로젝트 개요` 섹션만 수정 |
