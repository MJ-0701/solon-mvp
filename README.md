...
| Process | CEO 요구사항/plan → CTO/CPO sprint contract → CTO 구현 → CPO review → CTO rework/final confirm → retro. Planning sprint 는 G0/G1 이 두껍고, implementation sprint 는 상위 plan 을 인계받아 Do/G4 가 두껍다 |
...
| `bin/sfs` | global CLI entrypoint (`sfs init`, `sfs status`, `sfs plan` 등) |
