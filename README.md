...
4. 새 요구를 탐색하는 planning sprint 라면 `/sfs brainstorm` 으로 raw 요구사항을 Gate 2 (Brainstorm) 산출물에 남기고, Solon CEO 가 문제/제약/옵션/scope seed/plan seed 를 정리합니다. `/sfs start` 는 Guide 를 읽지 않은 사용자도 알 수 있도록 `simple` / default `normal` / `hard` brainstorm 선택지를 다음 줄에 보여줍니다. `--simple` 은 빠른 요구사항 정리, default normal 은 핵심 결정과 우선순위를 몇 가지 되묻는 기본 owner-thinking scaffold, `--hard` 는 product-owner hard training 입니다. raw 요구사항은 아직 계약이 아닙니다. 공유 design concept, domain language, feedback loop, interface/artifact boundary, gray-box 위임 경계가 비어 있으면 depth 에 맞게 blocking question 을 먼저 답합니다.
...
6. `/sfs implement "<첫 실행 slice>"` 에서 `implement.md` + `log.md` 를 열고, AI runtime 이 실제 작업 slice 와 검증 evidence 를 남깁니다. 코드 변경은 중요한 산출물이지만 유일한 산출물은 아닙니다. taxonomy, design handoff, QA evidence, infra/runbook, decision, docs 도 implementation artifact 입니다. 구현 모드는 공유 design concept, 도메인 용어, 작은 verification loop, 기존 프로젝트 규칙성을 기본 guardrail 로 삼습니다. Non-Dev 본부도 작은 기본값에서 시작해 trigger evidence 가 있을 때만 강화합니다: strategy-pm 은 lean PRD→roadmap/pricing, taxonomy 는 glossary→canonical model, design/frontend 는 existing system→tokens/component contract, QA 는 AC smoke→release readiness, infra 는 local/single deploy→SLO/runbook 으로 진행합니다. 코드가 포함될 때 DDD/TDD 와 backend Transaction discipline 이 강하게 적용됩니다. Backend architecture 는 MVP/소규모에서는 clean layered monolith, 초기 MVP 이후 backend 는 단일 DB 여도 CQRS 를 기본값으로 삼고, Hexagonal/MSA 전환은 안내 → 사용자 수용/승인 → incremental refactor 순서로만 진행합니다. Security / Infra / DevOps 는 sprint/project 단위 `light` / `full` / `skip` 질문으로만 확장하며, MVP-overkill 은 `deferred` 또는 `risk-accepted` 로 기록합니다.
7. `/sfs review` 로 CPO Evaluator artifact acceptance review 를 실행하고 verdict 를 남깁니다. Solon 은 plan/implement/log 와 변경 artifact 를 보고 Gate 와 review lens 를 자동 추론합니다. 코드 변경이면 code lens, 문서/전략/디자인/택소노미/QA/운영/릴리즈 작업이면 해당 산출물 수용성 lens 로 평가합니다. 추론이 틀렸을 때만 `--gate 6` 또는 `--lens docs|code|strategy|design|taxonomy|qa|ops|release` 로 override 합니다. 명령 출력은 lens/result path/verdict 메타데이터만 짧게 보여주고, AI runtime 은 원문 result 를 사용자의 언어로 요약해 해야 할 일을 Solon report 로 보여줍니다. 수동 handoff 만 필요하면 `--prompt-only` 를, 기존 리뷰 확인은 `--show-last` 를 사용합니다.
8. `/sfs report` 로 최종 작업보고서 `report.md` 를 만들고, `/sfs retro` 또는 `/sfs tidy --apply` 에서 workbench 문서를 archive 로 이동합니다. `retro` 는 기본적으로 회고+보고서 정리 후 sprint close + auto-commit 까지 수행합니다. 초안만 열고 싶을 때는 `/sfs retro --draft` 를 씁니다. `brainstorm/plan/implement/log/review` 는 작업 중 화이트보드이고, 완료 후에는 `report.md` + `retro.md` 가 읽는 입구입니다.
...
| `/sfs brainstorm [--simple|--easy|--quick|--hard] [text|--stdin]` | Gate 2 (Brainstorm) raw 요구사항/대화 맥락을 기록하고 depth 를 선택. `--simple` 은 빠른 정리, default normal 은 2~5개 핵심 질문으로 owner-thinking scaffold, `--hard` 는 의도/모순/우선순위/포기할 것/검증/경계/용어를 캐묻고 중요한 owner decision 이 비면 draft 유지 |
...
| `/sfs implement [work slice|--stdin]` | `implement.md`/`log.md` 를 열고 plan 기반 작업 slice 실행 + evidence 기록. 코드, taxonomy, design handoff, QA, infra/runbook, decision, docs 를 모두 artifact 로 취급하며, Non-Dev 본부는 lean PRD / glossary / existing design system / AC smoke / local deploy 에서 시작해 trigger evidence 가 있을 때만 강화. 코드가 포함될 때 backend Transaction / `REQUIRES_NEW` guardrail 을 always-on 으로 적용. Backend architecture 는 clean layered monolith → CQRS → Hexagonal 안내/수용 → MSA 안내/승인 ladder 로 기록. Security / Infra / DevOps 는 `light` / `full` / `skip`, 과한 항목은 `deferred` / `risk-accepted` 로 기록 |
| `/sfs review [--gate <1..7>] [--lens <auto|artifact|code|docs|strategy|design|taxonomy|qa|ops|release>] [--executor <tool>] [--prompt-only]` / `/sfs review --show-last` | CPO Evaluator artifact acceptance review run. 기본은 gate/lens 자동 추론이며 code review 는 code lens 일 때만 적용. stdout 은 lens/verdict/output path 메타데이터만 보여주고, AI runtime 이 result 를 사용자 언어의 요약+next action report 로 렌더링. full prompt 는 tmp prompt file에 저장하며 `review.md`에는 compact log/result를 남김 |
...
| `/sfs retro [--draft|--no-close]` | AI runtime 에서는 retro + report refinement 후, review 실행 여부 확인 + workbench/tmp archive + sprint close + auto commit. 이후 branch push/main 흡수는 Git Flow lifecycle 로 처리. 초안만 열려면 `--draft` |
...
- `/sfs retro` 는 사용자가 명시적으로 호출했을 때만 report 기반 workbench/tmp archive, sprint close, auto commit 을 수행합니다. 초안만 열려면 `/sfs retro --draft` 를 씁니다.
