...
| `.sfs-local/readiness-waiver` | "Sanity 상태를 알고도 map 을 진행한다" 를 기록하는 한 줄 waiver (사유 + 날짜). 있으면 doctor/map 이 advisory 대신 waiver 를 표시 |
...
| `llm-wiki/promotion-candidates/` | `sfs tidy --wiki-promote` 가 만드는 report/retro 계승 후보. 원본은 `docs/solon` 에 남고, wiki 는 source link 와 승격 checklist 만 받습니다. |
...
(AI-readiness 4축 채점과 Claude Code / Codex / Gemini 세션 비용 신호 포함 — 전부 신호만),
